...
static x264_frame_t *x264_frame_new(x264_t *h, int b_fdec) {
    ...
    for (int p = 0; p < luma_plane_count; p++) {
        int luma_plane_size = align_plane_size(
                frame->i_stride[p] * (frame->i_lines[p] + 2 * i_padv), disalign);
    ...
    if (b_fdec) /* fdec frame */
    {
        ...
        if (h->param.analyse.i_me_method >= X264_ME_ESA)
            PREALLOC(frame->buffer[3],
                     frame->i_stride[0] * (frame->i_lines[0] + 2 * i_padv) * sizeof(uint16_t)
    ...
    } else /* fenc frame */
    {
        if (h->frames.b_have_lowres) {
            int luma_plane_size = align_plane_size(
                    frame->i_stride_lowres * (frame->i_lines[0] / 2 + 2 * PADV), disalign);
    ...
    for (int p = 0; p < luma_plane_count; p++) {
        ...
        if (h->param.analyse.i_subpel_refine && b_fdec) {
            for (int i = 0; i < 4; i++) {
                frame->filtered[p][i] =
                        frame->buffer[p] + i * luma_plane_size + frame->i_stride[p] * i_padv + PADH;
                frame->filtered_fld[p][i] =
                        frame->buffer_fld[p] + i * luma_plane_size + frame->i_stride[p] * i_padv +
                        PADH;
            }
            frame->plane[p] = frame->filtered[p][0];
            frame->plane_fld[p] = frame->filtered_fld[p][0];
        } else {
            frame->filtered[p][0] = frame->plane[p] =
                    frame->buffer[p] + frame->i_stride[p] * i_padv + PADH;
            frame->filtered_fld[p][0] = frame->plane_fld[p] =
                    frame->buffer_fld[p] + frame->i_stride[p] * i_padv + PADH;
        }
    }
    ...
    if (b_fdec) {
        ...
        if (h->frames.b_have_lowres) {
            ...
            for (int i = 0; i < 4; i++)
                frame->lowres[i] =
                        frame->buffer_lowres[0] + (frame->i_stride_lowres * PADV + PADH) +
                        i * luma_plane_size;
...
static int
get_plane_ptr(x264_t *h, x264_picture_t *src, uint8_t **pix, int *stride, int plane, int xshift,
              int yshift) {
...
int x264_frame_copy_picture(x264_t *h, x264_frame_t *dst, x264_picture_t *src) {
    ...
    if (i_csp == X264_CSP_YUYV || i_csp == X264_CSP_UYVY) {
        ...
        h->mc.plane_copy_deinterleave_yuyv(dst->plane[p], dst->i_stride[p], dst->plane[p ^ 1],
                                           dst->i_stride[p ^ 1],
                                           (pixel *) src->img.plane[0], src->img.i_stride[0],
        ...
        h->mc.plane_copy_deinterleave_v210(dst->plane[0], dst->i_stride[0],
                                           ...
                                           (uint32_t *) pix[0], stride[0] / sizeof(uint32_t),
                                           h->param.i_width, h->param.i_height);
        ...
        h->mc.plane_copy_deinterleave_rgb(dst->plane[1 + b], dst->i_stride[1 + b],
                                          ...
                                          (pixel *) pix[0], stride[0] / sizeof(pixel),
                                          i_csp == X264_CSP_BGRA ? 4 : 3, h->param.i_width,
                                          h->param.i_height);
    } else {
        ...
        if (i_csp == X264_CSP_NV12 || i_csp == X264_CSP_NV16) {
            ...
            h->mc.plane_copy_swap(dst->plane[1], dst->i_stride[1], (pixel *) pix[1],
                                  stride[1] / sizeof(pixel), h->param.i_width >> 1,
                                  h->param.i_height >> v_shift);
        } else if (i_csp == X264_CSP_I420 || i_csp == X264_CSP_I422 || i_csp == X264_CSP_YV12 ||
                   i_csp == X264_CSP_YV16) {
...
plane_expand_border(pixel *pix, int i_stride, int i_width, int i_height, int i_padh, int i_padv,
                    int b_pad_top, int b_pad_bottom, int b_chroma) {
#define PPIXEL(x, y) ( pix + (x) + (y)*i_stride )
    for (int y = 0; y < i_height; y++) {
        /* left band */
        pixel_memset(PPIXEL(-i_padh, y), PPIXEL(0, y), i_padh >> b_chroma,
                     sizeof(pixel) << b_chroma);
        /* right band */
        pixel_memset(PPIXEL(i_width, y), PPIXEL(i_width - 1 - b_chroma, y), i_padh >> b_chroma,
                     sizeof(pixel) << b_chroma);
    }
    /* upper band */
    if (b_pad_top)
        for (int y = 0; y < i_padv; y++)
            memcpy(PPIXEL(-i_padh, -y - 1), PPIXEL(-i_padh, 0),
                   (i_width + 2 * i_padh) * sizeof(pixel));
    /* lower band */
    if (b_pad_bottom)
        for (int y = 0; y < i_padv; y++)
            memcpy(PPIXEL(-i_padh, i_height + y), PPIXEL(-i_padh, i_height - 1),
                   (i_width + 2 * i_padh) * sizeof(pixel));
#undef PPIXEL
}
...
void x264_frame_expand_border(x264_t *h, x264_frame_t *frame, int mb_y) {
    ...
    for (int i = 0; i < frame->i_plane; i++) {
        ...
        if (SLICE_MBAFF) {
            ...
            plane_expand_border(pix, stride * 2, width, height, padh, padv, pad_top, pad_bot,
                                h_shift);
            plane_expand_border(pix + stride, stride * 2, width, height, padh, padv, pad_top,
                                pad_bot, h_shift);
    ...
    for (int p = 0; p < (CHROMA444 ? 3 : 1); p++)
        for (int i = 1; i < 4; i++) {
            ...
            if (SLICE_MBAFF) {
                ...
                plane_expand_border(pix + stride, stride * 2, width, height, padh, padv, b_start,
                                    b_end, 0);
            }

            pix = frame->filtered[p][i] + (16 * mb_y - 8) * stride - 4;
            plane_expand_border(pix, stride, width, height << SLICE_MBAFF, padh, padv, b_start,
                                b_end, 0);
        }
}
...
void x264_frame_expand_border_lowres(x264_frame_t *frame) {
    for (int i = 0; i < 4; i++)
        plane_expand_border(frame->lowres[i], frame->i_stride_lowres, frame->i_width_lowres,
                            frame->i_lines_lowres, PADH, PADV, 1, 1, 0);
}

void x264_frame_expand_border_chroma(x264_t *h, x264_frame_t *frame, int plane) {
    int v_shift = CHROMA_V_SHIFT;
    plane_expand_border(frame->plane[plane], frame->i_stride[plane], 16 * h->mb.i_mb_width,
        ...
        if (i_pady) {
            for (int y = i_height; y < i_height + i_pady; y++)
                memcpy(&frame->plane[i][y * frame->i_stride[i]],
                       &frame->plane[i][(i_height - (~y & PARAM_INTERLACED) - 1) *
                                        frame->i_stride[i]],
...
void x264_weight_scale_plane(x264_t *h, pixel *dst, intptr_t i_dst_stride, pixel *src,
    ...
    while (i_height > 0) {
        ...
        for (x = 0; x < i_width - 8; x += 16)
            w->weightfn[16 >> 2](dst + x, i_dst_stride, src + x, i_src_stride, w,
                                 X264_MIN(i_height, 16));
        if (x < i_width)
            w->weightfn[8 >> 2](dst + x, i_dst_stride, src + x, i_src_stride, w,
                                X264_MIN(i_height, 16));
        i_height -= 16;
        dst += 16 * i_dst_stride;
        src += 16 * i_src_stride;
