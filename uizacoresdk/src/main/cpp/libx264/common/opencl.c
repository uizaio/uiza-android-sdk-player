...
static cl_program x264_opencl_cache_load(x264_t *h, const char *dev_name, const char *dev_vendor,
...
#undef CHECK_STRING
    ...
    cl_int status;
    program = ocl->clCreateProgramWithBinary(h->opencl.context, 1, &h->opencl.device, &size, &ptr,
                                             NULL, &status);
...
static void
x264_opencl_cache_save(x264_t *h, cl_program program, const char *dev_name, const char *dev_vendor,
                       const char *driver_version) {
    ...
    size_t size = 0;
    cl_int status = ocl->clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES, sizeof(size_t), &size,
                                          NULL);
...
static cl_program x264_opencl_compile(x264_t *h) {
    ...
    cl_int status;
    status = ocl->clGetDeviceInfo(h->opencl.device, CL_DEVICE_NAME, sizeof(dev_name), dev_name,
                                  NULL);
    status |= ocl->clGetDeviceInfo(h->opencl.device, CL_DEVICE_VENDOR, sizeof(dev_vendor),
                                   dev_vendor, NULL);
    status |= ocl->clGetDeviceInfo(h->opencl.device, CL_DRIVER_VERSION, sizeof(driver_version),
                                   driver_version, NULL);
    if (status != CL_SUCCESS)
        return NULL;
    ...
    if (vectorize) {
        ...
        cl_uint simdwidth = 4;
        status = ocl->clGetDeviceInfo(h->opencl.device, CL_DEVICE_SIMD_INSTRUCTION_WIDTH_AMD,
                                      sizeof(cl_uint), &simdwidth, NULL);
    ...
    status = ocl->clGetProgramBuildInfo(program, h->opencl.device, CL_PROGRAM_BUILD_LOG, 0, NULL,
                                        &build_log_len);
    ...
    status = ocl->clGetProgramBuildInfo(program, h->opencl.device, CL_PROGRAM_BUILD_LOG,
...
static int x264_opencl_lookahead_alloc(x264_t *h) {
    ...
    h->opencl.page_locked_buffer = ocl->clCreateBuffer(h->opencl.context,
                                                       CL_MEM_WRITE_ONLY | CL_MEM_ALLOC_HOST_PTR,
                                                       PAGE_LOCKED_BUF_SIZE, NULL, &status);
    if (status != CL_SUCCESS) {
        x264_log(h, X264_LOG_WARNING, "OpenCL: Unable to allocate page-locked buffer, error '%d'\n",
                 status);
        goto fail;
    }
    h->opencl.page_locked_ptr = ocl->clEnqueueMapBuffer(h->opencl.queue,
                                                        h->opencl.page_locked_buffer, CL_TRUE,
                                                        CL_MAP_READ | CL_MAP_WRITE,
                                                        0, PAGE_LOCKED_BUF_SIZE, 0, NULL, NULL,
                                                        &status);
    if (status != CL_SUCCESS) {
...
x264_opencl_error_notify(const char *errinfo, const void *private_info, size_t cb,
                         void *user_data) {
...
int x264_opencl_lookahead_init(x264_t *h) {
    ...
    for (cl_uint i = 0; i < numPlatforms; i++) {
        ...
        for (cl_uint gpu = 0; gpu < gpu_count; gpu++) {
            ...
            if (h->param.opencl_device_id &&
                devices[gpu] != (cl_device_id) h->param.opencl_device_id)
                continue;

            cl_bool image_support = 0;
            status = ocl->clGetDeviceInfo(h->opencl.device, CL_DEVICE_IMAGE_SUPPORT,
            ...
            context = ocl->clCreateContext(NULL, 1, &h->opencl.device,
                                           (void *) x264_opencl_error_notify, (void *) h, &status);
            if (status != CL_SUCCESS || !context)
                continue;

            cl_uint imagecount = 0;
            status = ocl->clGetSupportedImageFormats(context, CL_MEM_READ_WRITE,
            ...
            status = ocl->clGetSupportedImageFormats(context, CL_MEM_READ_WRITE,
                                                     CL_MEM_OBJECT_IMAGE2D, imagecount, imageType,
                                                     NULL);
            if (status != CL_SUCCESS)
                continue;
            ...
            if (!b_has_r || !b_has_rgba) {
                char dev_name[64];
                status = ocl->clGetDeviceInfo(h->opencl.device, CL_DEVICE_NAME, sizeof(dev_name),
                                              dev_name, NULL);
                if (status == CL_SUCCESS) {
...
static int x264_detect_switchable_graphics(void) {
    ...
    ADL_Main_Control_Create = (ADL_MAIN_CONTROL_CREATE) adl_address(hDLL,
                                                                    "ADL_Main_Control_Create");
    ADL_Main_Control_Destroy = (ADL_MAIN_CONTROL_DESTROY) adl_address(hDLL,
                                                                      "ADL_Main_Control_Destroy");
    ADL_Adapter_NumberOfAdapters_Get = (ADL_ADAPTER_NUMBEROFADAPTERS_GET) adl_address(hDLL,
                                                                                      "ADL_Adapter_NumberOfAdapters_Get");
    ADL_PowerXpress_Scheme_Get = (ADL_POWERXPRESS_SCHEME_GET) adl_address(hDLL,
    ...
    for (int i = 0; i < numAdapters; i++) {
        ...
        if (ADL_OK != ADL_PowerXpress_Scheme_Get(i, &PXSchemeRange, &PXSchemeCurrentState,
                                                 &PXSchemeDefaultState))
            break;
