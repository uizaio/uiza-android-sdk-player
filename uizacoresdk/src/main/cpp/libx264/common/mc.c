...
static void
mc_copy(pixel *src, intptr_t i_src_stride, pixel *dst, intptr_t i_dst_stride, int i_width,
        int i_height) {
    for (int y = 0; y < i_height; y++) {
...
static void load_deinterleave_chroma_fenc(pixel *dst, pixel *src, intptr_t i_src, int height) {
    x264_plane_copy_deinterleave_c(dst, FENC_STRIDE, dst + FENC_STRIDE / 2, FENC_STRIDE, src, i_src,
                                   8, height);
}

static void load_deinterleave_chroma_fdec(pixel *dst, pixel *src, intptr_t i_src, int height) {
    x264_plane_copy_deinterleave_c(dst, FDEC_STRIDE, dst + FDEC_STRIDE / 2, FDEC_STRIDE, src, i_src,
                                   8, height);
}
...
void x264_frame_init_lowres(x264_t *h, x264_frame_t *frame) {
    ...
    memcpy(src + i_stride * i_height, src + i_stride * (i_height - 1),
           (i_width + 1) * sizeof(pixel));
    h->mc.frame_init_lowres_core(src, frame->lowres[0], frame->lowres[1], frame->lowres[2],
                                 frame->lowres[3],
                                 i_stride, frame->i_stride_lowres, frame->i_width_lowres,
...
static void frame_init_lowres_core(pixel *src0, pixel *dst0, pixel *dsth, pixel *dstv, pixel *dstc,
                                   intptr_t src_stride, intptr_t dst_stride, int width,
                                   int height) {
    for (int y = 0; y < height; y++) {
...
void x264_frame_filter(x264_t *h, x264_frame_t *frame, int mb_y, int b_end) {
    ...
    int height =
            (b_end ? frame->i_lines[0] + 16 * PARAM_INTERLACED : (mb_y + b_interlaced) * 16) + 8;
