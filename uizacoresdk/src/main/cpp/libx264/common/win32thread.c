...
ret = NULL;
thread->
handle = (void *) _beginthreadex(NULL, 0, x264_win32thread_worker, thread, 0, NULL);
return !thread->
handle;
}

int x264_pthread_join(x264_pthread_t
...
if( !
memcmp( mutex,
&init, sizeof(x264_pthread_mutex_t)))
*
mutex = static_mutex;
...
static struct {
    ...
    BOOL (WINAPI *cond_wait)( x264_pthread_cond_t *cond,
    x264_pthread_mutex_t *mutex, DWORD
    milliseconds );
} thread_control;
...
Ptr = win32_cond;
win32_cond->
semaphore = CreateSemaphoreW(NULL, 0, 0x7fffffff, NULL);
...
win32_cond->
waiters_done = CreateEventW(NULL, FALSE, FALSE, NULL);
...
if( have_waiter )
{
ReleaseSemaphore( win32_cond
->semaphore, win32_cond->waiter_count, NULL );
x264_pthread_mutex_unlock(&win32_cond->mtx_waiter_count);
WaitForSingleObject( win32_cond
...
int x264_win32_threading_init(void) {
    ...
    if (thread_control.cond_init) {
        ...
        thread_control.cond_broadcast = (void *) GetProcAddress(kernel_dll,
