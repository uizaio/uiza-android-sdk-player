...
int x264_cqm_init(x264_t *h) {
    ...
    int num_8x8_lists = h->sps->i_chroma_format_idc == CHROMA_444 ? 4
                                                                  : h->param.analyse.b_transform_8x8
                                                                    ? 2
                                                                    : 0; /* Checkasm may segfault if optimized out by --chroma-format */
...
#define CQM_ALLOC(w, count)\
    ...
    for (int q = 0; q < 6; q++) {
        for (int i_list = 0; i_list < 4; i_list++)
            for (int i = 0; i < 16; i++) {
                ...
                quant4_mf[i_list][q][i] = DIV(def_quant4[q][i] * 16,
                                              h->pps->scaling_list[i_list][i]);
            }
        for (int i_list = 0; i_list < num_8x8_lists; i_list++)
            for (int i = 0; i < 64; i++) {
                h->dequant8_mf[i_list][q][i] =
                        def_dequant8[q][i] * h->pps->scaling_list[4 + i_list][i];
                quant8_mf[i_list][q][i] = DIV(def_quant8[q][i] * 16,
                                              h->pps->scaling_list[4 + i_list][i]);
            }
    }
    for (int q = 0; q <= QP_MAX_SPEC; q++) {
        ...
        for (int i_list = 0; i_list < 4; i_list++)
            for (int i = 0; i < 16; i++) {
                h->unquant4_mf[i_list][q][i] =
                        (1ULL << (q / 6 + 15 + 8)) / quant4_mf[i_list][q % 6][i];
                ...
                h->quant4_bias[i_list][q][i] = X264_MIN(DIV(deadzone[i_list] << 10, j),
                                                        (1 << 15) / j);
        ...
        if (h->param.analyse.b_transform_8x8)
            for (int i_list = 0; i_list < num_8x8_lists; i_list++)
                for (int i = 0; i < 64; i++) {
                    h->unquant8_mf[i_list][q][i] =
                            (1ULL << (q / 6 + 16 + 8)) / quant8_mf[i_list][q % 6][i];
                    ...
                    h->quant8_bias[i_list][q][i] = X264_MIN(DIV(deadzone[i_list] << 10, j),
                                                            (1 << 15) / j);
                    h->quant8_bias0[i_list][q][i] = (1 << 15) / j;
                    if (j > 0xffff && q > max_qp_err && (i_list == CQM_8IY || i_list == CQM_8PY))
                        max_qp_err = q;
                    if (j > 0xffff && q > max_chroma_qp_err &&
                        (i_list == CQM_8IC || i_list == CQM_8PC))
                        max_chroma_qp_err = q;
                }
    }
    ...
    x264_emms();
    CHECKED_MALLOC(h->nr_offset_emergency,
                   sizeof(*h->nr_offset_emergency) * (QP_MAX - QP_MAX_SPEC));
    ...
    if (!h->mb.b_lossless) {
        ...
        if (!h->param.b_cabac && h->sps->i_profile_idc < PROFILE_HIGH)
            while (h->chroma_qp_table[SPEC_QP(h->param.rc.i_qp_max)] <= 12 ||
...
int x264_cqm_parse_file(x264_t *h, const char *filename) {
    ...
    b_error |= x264_cqm_parse_jmlist(h, buf, "INTRA4X4_CHROMA", h->param.cqm_4ic, x264_cqm_jvt4i,
                                     16);
    b_error |= x264_cqm_parse_jmlist(h, buf, "INTER4X4_CHROMA", h->param.cqm_4pc, x264_cqm_jvt4p,
                                     16);
    ...
    if (CHROMA444) {
        b_error |= x264_cqm_parse_jmlist(h, buf, "INTRA8X8_CHROMA", h->param.cqm_8ic,
                                         x264_cqm_jvt8i, 64);
        b_error |= x264_cqm_parse_jmlist(h, buf, "INTER8X8_CHROMA", h->param.cqm_8pc,
                                         x264_cqm_jvt8p, 64);
    }
