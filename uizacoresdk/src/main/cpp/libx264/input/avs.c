...
static void
avs_build_filter_sequence(char *filename_ext, const char *filter[AVS_MAX_SEQUENCE + 1]) {
    int i = 0;
#if USE_AVXSYNTH
...
static AVS_Value
update_clip(avs_hnd_t *h, const AVS_VideoInfo **vi, AVS_Value res, AVS_Value release) {
...
/* AvxSynth has its version defined starting at 4.0, even though it's based on
   ...
   the version as 2.58. */
    ...
    return 2.58f;
#else
    FAIL_IF_ERROR(!h->func.avs_function_exists(h->env, "VersionNumber"),
                  "VersionNumber does not exist\n");
...
static int
open_file(char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt) {
    ...
    if (avs_is_field_based(vi)) {
        x264_cli_log("avs", X264_LOG_WARNING,
    ...
    FAIL_IF_ERROR(avs_version < 2.6f &&
                  (opt->output_csp == X264_CSP_I422 || opt->output_csp == X264_CSP_I444),
    ...
    if ((opt->output_csp == X264_CSP_I420 && !AVS_IS_420(vi)) ||
        ...
        if (avs_version >= 2.56f && ((opt->output_csp == X264_CSP_RGB && avs_is_yuv(vi)) ||
                                     (opt->output_csp != X264_CSP_RGB && avs_is_rgb(vi)))) {
            // if converting from yuv, then we specify the matrix for the input, otherwise use the output's.
            int use_pc_matrix = avs_is_yuv(vi) ? opt->input_range == RANGE_PC : opt->output_range ==
                                                                                RANGE_PC;
            snprintf(matrix, sizeof(matrix), "%s601",
                     use_pc_matrix ? "PC." : "Rec"); /* FIXME: use correct coefficients */
            arg_count++;
        ...
        AVS_Value res2 = h->func.avs_invoke(h->env, conv_func,
                                            avs_new_value_array(arg_arr, arg_count), arg_name);
    ...
    if (avs_is_yuv(vi) && opt->output_range != RANGE_AUTO &&
        ((opt->input_range == RANGE_PC) != opt->output_range)) {
        ...
        AVS_Value res2 = h->func.avs_invoke(h->env, "ColorYUV", avs_new_value_array(arg_arr, 2),
                                            arg_name);
    ...
    if (AVS_IS_RGB64(vi))
        ...
        const char *pixel_type_name = avs_is_string(pixel_type) ? avs_as_string(pixel_type)
                                                                : "unknown";
...
const cli_input_t avs_input = {open_file, picture_alloc, read_frame, release_frame, picture_clean,
                               close_file};
