...
open_file(char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt) {
    ...
    FAIL_IF_ERR(
            !h || cli_input.picture_alloc(&h->pic, *p_handle, info->csp, info->width, info->height),
            "x264", "malloc failed\n");
    h->input = cli_input;
    h->p_handle = *p_handle;
    h->next_frame = -1;
...
const cli_input_t thread_input = {open_file, picture_alloc, read_frame, release_frame,
