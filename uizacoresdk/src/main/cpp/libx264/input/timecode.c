...
static int try_mkv_timebase_den(double *fpss, timecode_hnd_t *h, int loop_num) {
    ...
    for (int num = 0; num < loop_num; num++) {
        ...
        FAIL_IF_ERROR(h->timebase_num > UINT32_MAX || !h->timebase_num,
...
#define NO_TIMECODE_LINE (buff[0] == '#' || buff[0] == '\n' || buff[0] == '\r')
    if (tcfv == 1) {
        ...
        for (num = 2; fgets(buff, sizeof(buff), tcfile_in) != NULL; num++) {
            if (NO_TIMECODE_LINE)
                continue;
            FAIL_IF_ERROR(sscanf(buff, "assume %lf", &h->assume_fps) != 1 &&
                          sscanf(buff, "Assume %lf", &h->assume_fps) != 1,
        ...
        timecodes[0] = 0;
        for (num = seq_num = 0;
             num < timecodes_num - 1 && fgets(buff, sizeof(buff), tcfile_in) != NULL;) {
            if (NO_TIMECODE_LINE)
                continue;
            ret = sscanf(buff, "%d,%d,%lf", &start, &end, &seq_fps);
            if (ret != 3)
                ...
                if (h->auto_timebase_den) {
                    ...
                    while (1) {
                        ...
                        if (fps_num > UINT32_MAX ||
                            fabs(((double) fps_num / fps_den) / exponent - fps_sig) <
                            DOUBLE_EPSILON)
                            break;
                        ++i;
                    }
                    h->timebase_den =
                            fps_num && h->timebase_den ? lcm(h->timebase_den, fps_num) : fps_num;
                    if (h->timebase_den > UINT32_MAX) {
                        h->auto_timebase_den = 0;
                        continue;
                    }
...
#undef NO_TIMECODE_LINE
    if (h->auto_timebase_den || h->auto_timebase_num) {
        ...
        h->timebase_num /= i;
        h->timebase_den /= i;
        x264_cli_log("timecode", X264_LOG_INFO, "automatic timebase generation %"
        PRIu64
        "/%"
        PRIu64
        "\n", h->timebase_num, h->timebase_den );
    } else
        FAIL_IF_ERROR(h->timebase_den > UINT32_MAX || !h->timebase_den,
                      "automatic timebase generation failed.\n"
    ...
    for (num = 0; num < h->stored_pts_num; num++) {
        ...
        FAIL_IF_ERROR(num > 0 && h->pts[num] <= h->pts[num - 1],
                      "invalid timebase or timecode for frame %d\n", num);
    }
...
open_file(char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt) {
    ...
    h->pts = NULL;
    if (opt->timebase) {
        ret = sscanf(opt->timebase, "%"
        SCNu64
        "/%"
        SCNu64, &h->timebase_num, &h->timebase_den );
        if (ret == 1) {
    ...
    if (!x264_is_regular_file(tcfile_in)) {
        x264_cli_log("timecode", X264_LOG_ERROR,
...
static int64_t get_frame_pts(timecode_hnd_t *h, int frame, int real_frame) {
    if (frame < h->stored_pts_num)
        ...
        if (h->pts && real_frame) {
            x264_cli_log("timecode", X264_LOG_INFO,
...
const cli_input_t timecode_input = {open_file, picture_alloc, read_frame, release_frame,
                                    picture_clean, close_file};
