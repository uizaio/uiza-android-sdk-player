...
static int parse_csp_and_depth(char *csp_name, int *bit_depth) {
    ...
    if (!strncmp("420", csp_name, 3))
        ...
        csp = X264_CSP_I422;
             ...
             strncmp("444alpha", csp_name, 8)) // only accept alphaless 4:4:4
        csp = X264_CSP_I444;
...
open_file(char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt) {
    ...
    FAIL_IF_ERROR(colorspace <= X264_CSP_NONE || colorspace >= X264_CSP_MAX,
    ...
    if (x264_is_regular_file(h->fh)) {
        ...
        while (len <= MAX_FRAME_HEADER && fgetc(h->fh) != '\n')
            len++;
        FAIL_IF_ERROR(len > MAX_FRAME_HEADER || len < sizeof(Y4M_FRAME_MAGIC),
                      "bad frame header length\n");
...
static int read_frame_internal(cli_pic_t *pic, y4m_hnd_t *h, int bit_depth_uc) {
    ...
    for (i = 0; i < pic->img.planes; i++) {
        ...
        } else if (fread(pic->img.plane[i], pixel_depth, h->plane_size[i], h->fh) !=
                   h->plane_size[i])
            return -1;
...
static int read_frame(cli_pic_t *pic, hnd_t handle, int i_frame) {
    ...
    if (h->use_mmap) {
        pic->img.plane[0] = x264_cli_mmap(&h->mmap, h->frame_size * i_frame + h->seq_header_len,
                                          h->frame_size);
        if (!pic->img.plane[0])
            return -1;
...
const cli_input_t y4m_input = {open_file, picture_alloc, read_frame, release_frame, picture_clean,
                               close_file};
