...
open_file(char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt) {
    ...
    if (opt->resolution) {
        ...
        const char *csp = opt->colorspace ? opt->colorspace : av_get_pix_fmt_name(
                AV_PIX_FMT_YUV420P);
    ...
    FAIL_IF_ERROR(avformat_open_input(&h->lavf, psz_filename, format, &options),
                  "could not open input file\n");
    if (options)
        av_dict_free(&options);
    FAIL_IF_ERROR(avformat_find_stream_info(h->lavf, NULL) < 0,
                  "could not find input stream info\n");
    ...
    FAIL_IF_ERROR(!h->first_pic ||
                  lavf_input.picture_alloc(h->first_pic, h, X264_CSP_OTHER, info->width,
                                           info->height),
                  "malloc failed\n");
...
const cli_input_t lavf_input = {open_file, picture_alloc, read_frame, NULL, picture_clean,
                                close_file};
