...
open_file(char *psz_filename, hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt) {
    ...
    if (opt->colorspace) {
        for (info->csp = X264_CSP_CLI_MAX - 1; info->csp > X264_CSP_NONE; info->csp--) {
            if (x264_cli_csps[info->csp].name &&
                !strcasecmp(x264_cli_csps[info->csp].name, opt->colorspace))
                break;
        }
...
const cli_input_t raw_input = {open_file, picture_alloc, read_frame, release_frame, picture_clean,
                               close_file};
