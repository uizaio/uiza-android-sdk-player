...
static int parse(int argc, char **argv, x264_param_t *param, cli_opt_t *opt) {
    ...
    if (!tcfile_name && input_opt.timebase) {
        ...
        int ret = sscanf(input_opt.timebase, "%"
        SCNu64
        "/%"
        SCNu64, &i_user_timebase_num, &i_user_timebase_den );
...
static int64_t
print_status(int64_t i_start, int64_t i_previous, int i_frame, int i_frame_total, int64_t i_file,
             x264_param_t *param, int64_t last_ts) {
    char buf[200];
    ...
    for (; !b_ctrl_c && (i_frame < param->i_frame_total || !param->i_frame_total); i_frame++) {
        ...
        if (pic.i_pts <= largest_pts) {
            if (cli_log_level >= X264_LOG_DEBUG || pts_warning_cnt < MAX_PTS_WARNING)
                x264_cli_log("x264", X264_LOG_WARNING, "non-strictly-monotonic pts at frame %d (%"
            PRId64
            " <= %"
            PRId64
            ")\n",
