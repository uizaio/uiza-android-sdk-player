...
static int close_file(hnd_t handle, int64_t largest_pts, int64_t second_largest_pts) {
    ...
    if (p_mp4->p_root) {
        ...
        if (p_mp4->i_track) {
            ...
            MP4_LOG_IF_ERR(lsmash_flush_pooled_samples(p_mp4->p_root, p_mp4->i_track,
                                                       (last_delta ? last_delta : 1) *
                                                       p_mp4->i_time_inc),
                           "failed to flush the rest of samples.\n");

            if (p_mp4->i_movie_timescale != 0 &&
                p_mp4->i_video_timescale != 0)    /* avoid zero division */
                actual_duration = ((double) ((largest_pts + last_delta) * p_mp4->i_time_inc) /
                                   p_mp4->i_video_timescale) * p_mp4->i_movie_timescale;
            else
            ...
            if (!p_mp4->b_fragments) {
                MP4_LOG_IF_ERR(
                        lsmash_create_explicit_timeline_map(p_mp4->p_root, p_mp4->i_track, edit),
                        "failed to set timeline map for video.\n");
            } else if (!p_mp4->b_stdout)
                MP4_LOG_IF_ERR(
                        lsmash_modify_explicit_timeline_map(p_mp4->p_root, p_mp4->i_track, 1, edit),
...
static int open_file(char *psz_filename, hnd_t *p_handle, cli_output_opt_t *opt) {
    *p_handle = NULL;
    ...
    MP4_FAIL_IF_ERR_EX(lsmash_open_file(psz_filename, 0, &p_mp4->file_param) < 0,
...
static int set_param(hnd_t handle, x264_param_t *p_param) {
    ...
    MP4_FAIL_IF_ERR(!lsmash_set_file(p_mp4->p_root, file_param),
                    "failed to add an output file into a ROOT.\n");
    ...
    p_mp4->summary->color.matrix_index = p_param->vui.i_colmatrix >= 0 ? p_param->vui.i_colmatrix
...
static int write_frame(hnd_t handle, uint8_t *p_nalu, int i_size, x264_picture_t *p_picture) {
    ...
    p_sample->prop.ra_flags = p_picture->b_keyframe ? ISOM_SAMPLE_RANDOM_ACCESS_FLAG_SYNC
    ...
    if (p_mp4->b_fragments && p_mp4->i_numframe &&
        ...
        MP4_FAIL_IF_ERR(lsmash_flush_pooled_samples(p_mp4->p_root, p_mp4->i_track,
                                                    p_sample->dts - p_mp4->i_prev_dts),
                        "failed to flush the rest of samples.\n");
