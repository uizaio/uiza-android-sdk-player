...
#define convert_timebase_ms(timestamp, timebase) (int64_t)((timestamp) * (timebase) * 1000 + 0.5)
    ...
    if (!p_flv->i_framenum) {
        ...
        if (!p_flv->b_dts_compress && p_flv->i_delay_time)
            x264_cli_log("flv", X264_LOG_INFO, "initial delay %"
        PRId64
        " ms\n",
    ...
    if (p_flv->b_dts_compress) {
        if (p_flv->i_framenum == 1)
            p_flv->i_init_delta = convert_timebase_ms(p_picture->i_dts + p_flv->i_delay_time,
                                                      p_flv->d_timebase);
    ...
    if (p_flv->i_framenum) {
        if (p_flv->i_prev_dts == dts)
            x264_cli_log("flv", X264_LOG_WARNING, "duplicate DTS %"
        PRId64
        " generated by rounding\n"
                "               decoding framerate cannot exceed 1000fps\n", dts );
        if (p_flv->i_prev_cts == cts)
            x264_cli_log("flv", X264_LOG_WARNING, "duplicate CTS %"
        PRId64
        " generated by rounding\n"
...
static int close_file(hnd_t handle, int64_t largest_pts, int64_t second_largest_pts) {
    ...
    if (x264_is_regular_file(c->fp) && total_duration > 0) {
        ...
        CHECK(rewrite_amf_double(c->fp, p_flv->i_bitrate_pos,
                                 filesize * 8 / (total_duration * 1000)));
    }
    ret = 0;
