...
const static uint8_t avcintra_uuid[] = {0xF7, 0x49, 0x3E, 0xB3, 0xD4, 0x00, 0x47, 0x96, 0x86, 0x86,
                                        0xC9, 0x70, 0x7B, 0x64, 0x37, 0x2A};
...
static void scaling_list_write(bs_t *s, x264_pps_t *pps, int idx) {
    ...
    const uint8_t *def_list = (idx == CQM_4IC) ? pps->scaling_list[CQM_4IY]
                                               : (idx == CQM_4PC) ? pps->scaling_list[CQM_4PY]
                                                                  : (idx == CQM_8IC + 4)
                                                                    ? pps->scaling_list[CQM_8IY + 4]
                                                                    : (idx == CQM_8PC + 4)
                                                                      ? pps->scaling_list[CQM_8PY +
                                                                                          4]
                                                                      : x264_cqm_jvt[idx];
...
void x264_sps_init(x264_sps_t *sps, int i_id, x264_param_t *param) {
    ...
    if (sps->b_qpprime_y_zero_transform_bypass || sps->i_chroma_format_idc == CHROMA_444)
        ...
        sps->i_profile_idc = PROFILE_HIGH;
             ...
             param->b_fake_interlaced || param->analyse.i_weighted_pred > 0)
    ...
    if (param->i_level_idc == 9 &&
        (sps->i_profile_idc == PROFILE_BASELINE || sps->i_profile_idc == PROFILE_MAIN)) {
    ...
    if (param->b_intra_refresh) {
        int time_to_recovery =
                X264_MIN(sps->i_mb_width - 1, param->i_keyint_max) + param->i_bframe - 1;
    ...
    sps->vui.i_vidformat = (param->vui.i_vidformat >= 0 && param->vui.i_vidformat <= 5
                            ? param->vui.i_vidformat : 5);
    sps->vui.b_fullrange = (param->vui.b_fullrange >= 0 && param->vui.b_fullrange <= 1
                            ? param->vui.b_fullrange :
                            (csp >= X264_CSP_BGR ? 1 : 0));
    sps->vui.b_color_description_present = 0;

    sps->vui.i_colorprim = (param->vui.i_colorprim >= 0 && param->vui.i_colorprim <= 12
                            ? param->vui.i_colorprim : 2);
    sps->vui.i_transfer = (param->vui.i_transfer >= 0 && param->vui.i_transfer <= 17
                           ? param->vui.i_transfer : 2);
    sps->vui.i_colmatrix = (param->vui.i_colmatrix >= 0 && param->vui.i_colmatrix <= 11
    ...
    sps->vui.b_chroma_loc_info_present =
            param->vui.i_chroma_loc > 0 && param->vui.i_chroma_loc <= 5 &&
    ...
    sps->vui.b_bitstream_restriction = !(sps->b_constraint_set3 &&
                                         sps->i_profile_idc >= PROFILE_HIGH);
...
void x264_sps_write(bs_t *s, x264_sps_t *sps) {
    ...
    if (sps->b_crop) {
        int h_shift =
                sps->i_chroma_format_idc == CHROMA_420 || sps->i_chroma_format_idc == CHROMA_422;
...
void x264_pps_init(x264_pps_t *pps, int i_id, x264_param_t *param, x264_sps_t *sps) {
    ...
    pps->i_pic_init_qp =
            param->rc.i_rc_method == X264_RC_ABR || param->b_stitchable ? 26 + QP_BD_OFFSET
                                                                        : SPEC_QP(
...
void x264_sei_buffering_period_write(x264_t *h, bs_t *s) {
    ...
    if (sps->vui.b_nal_hrd_parameters_present) {
        ...
        bs_write(&q, sps->vui.hrd.i_initial_cpb_removal_delay_length,
...
void x264_sei_pic_timing_write(x264_t *h, bs_t *s) {
    ...
    if (sps->vui.b_nal_hrd_parameters_present || sps->vui.b_vcl_hrd_parameters_present) {
        bs_write(&q, sps->vui.hrd.i_cpb_removal_delay_length,
...
void x264_sei_frame_packing_write(x264_t *h, bs_t *s) {
    ...
    bs_write1(&q, h->param.i_frame_packing == 5 &&
                  !(h->fenc->i_frame & 1)); // current_frame_is_frame0_flag
...
int x264_validate_levels(x264_t *h, int verbose) {
    ...
    if (l->frame_size < mbs
        ...
        || l->frame_size * 8 < h->sps->i_mb_height * h->sps->i_mb_height) ERROR(
            "frame MB size (%dx%d) > level limit (%d)\n",
            h->sps->i_mb_width, h->sps->i_mb_height, l->frame_size);
    if (dpb > l->dpb) ERROR("DPB size (%d frames, %d mbs) > level limit (%d frames, %d mbs)\n",
