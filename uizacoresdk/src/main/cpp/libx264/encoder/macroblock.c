...
static inline void
idct_dequant_2x2_dc(dctcoef dct[4], dctcoef dct4x4[4][16], int dequant_mf[6][16], int i_qp) {
...
static void x264_mb_encode_i16x16(x264_t *h, int p, int i_qp) {
    ...
    if (h->mb.b_lossless) {
        for (int i = 0; i < 16; i++) {
            ...
            nz = h->zigzagf.sub_4x4ac(h->dct.luma4x4[16 * p + i], p_src + oe, p_dst + od,
                                      &dct_dc4x4[block_idx_yx_1d[i]]);
    ...
    if (h->mb.b_trellis) {
        for (int idx = 0; idx < 16; idx++)
            if (x264_quant_4x4_trellis(h, dct4x4[idx], i_quant_cat, i_qp,
                                       ctx_cat_plane[DCT_LUMA_AC][p], 1, !!p, idx)) {
                block_cbp = 0xf;
                h->zigzagf.scan_4x4(h->dct.luma4x4[16 * p + idx], dct4x4[idx]);
                h->quantf.dequant_4x4(dct4x4[idx], h->dequant4_mf[i_quant_cat], i_qp);
                if (decimate_score < 6)
                    decimate_score += h->quantf.decimate_score15(h->dct.luma4x4[16 * p + idx]);
                h->mb.cache.non_zero_count[x264_scan8[16 * p + idx]] = 1;
            }
    } else {
        for (int i8x8 = 0; i8x8 < 4; i8x8++) {
            nz = h->quantf.quant_4x4x4(&dct4x4[i8x8 * 4], h->quant4_mf[i_quant_cat][i_qp],
                                       h->quant4_bias[i_quant_cat][i_qp]);
            if (nz) {
                block_cbp = 0xf;
    ...
    if (h->mb.b_trellis)
        nz = x264_quant_luma_dc_trellis(h, dct_dc4x4, i_quant_cat, i_qp,
                                        ctx_cat_plane[DCT_LUMA_DC][p], 1, LUMA_DC + p);
    else
        nz = h->quantf.quant_4x4_dc(dct_dc4x4, h->quant4_mf[i_quant_cat][i_qp][0] >> 1,
                                    h->quant4_bias[i_quant_cat][i_qp][0] << 1);
    ...
    if (nz) {
        ...
        h->quantf.dequant_4x4_dc(dct_dc4x4, h->dequant4_mf[i_quant_cat],
                                 i_qp);  /* XXX not inversed */
        if (block_cbp)
            for (int i = 0; i < 16; i++)
...
x264_mb_optimize_chroma_dc(x264_t *h, dctcoef *dct_dc, int dequant_mf[6][16], int i_qp,
                           int chroma422) {
    int dmf = dequant_mf[i_qp % 6][0] << i_qp / 6;
...
x264_mb_encode_chroma_internal(x264_t *h, int b_inter, int i_qp, int chroma422) {
    int nz, nz_dc;
    ...
    if (b_decimate && i_qp >= (h->mb.b_trellis ? 12 : 18) && !h->mb.b_noise_reduction) {
        int thresh = chroma422 ? (x264_lambda2_tab[i_qp] + 16) >> 5 : (x264_lambda2_tab[i_qp] + 32)
                >> 6;
        ALIGNED_ARRAY_8(
        int, ssd,[2] );
        ...
        if (h->pixf.var2[chromapix](h->mb.pic.p_fenc[1], h->mb.pic.p_fdec[1], ssd) < thresh * 4) {
            ...
            for (int ch = 0; ch < 2; ch++) {
                if (ssd[ch] > thresh) {
                    ...
                    if (h->mb.b_trellis)
                        nz_dc = x264_quant_chroma_dc_trellis(h, dct_dc, i_qp + 3 * chroma422,
                                                             !b_inter, CHROMA_DC + ch);
                    else {
                        nz_dc = 0;
                        for (int i = 0; i <= chroma422; i++)
                            nz_dc |= h->quantf.quant_2x2_dc(&dct_dc[4 * i],
                                                            h->quant4_mf[CQM_4IC + b_inter][i_qp +
                                                                                            3 *
                                                                                            chroma422][0]
                                                                    >> 1,
                    ...
                    if (nz_dc) {
                        if (!x264_mb_optimize_chroma_dc(h, dct_dc, dequant_mf, i_qp + 3 * chroma422,
                                                        chroma422))
                            continue;
    ...
    for (int ch = 0; ch < 2; ch++) {
        ...
        if (h->mb.b_lossless) {
            ...
            for (int i = 0; i < (chroma422 ? 8 : 4); i++) {
                ...
                nz = h->zigzagf.sub_4x4ac(
                        h->dct.luma4x4[16 + i + (chroma422 ? i & 4 : 0) + ch * 16], p_src + oe,
                        p_dst + od,
                        &h->dct.chroma_dc[ch][chroma422 ? chroma422_scan[i] : i]);
                h->mb.cache.non_zero_count[x264_scan8[16 + i + (chroma422 ? i & 4 : 0) +
                                                      ch * 16]] = nz;
                h->mb.i_cbp_chroma |= nz;
            }
            h->mb.cache.non_zero_count[x264_scan8[CHROMA_DC + ch]] = array_non_zero(
                    h->dct.chroma_dc[ch], chroma422 ? 8 : 4);
            continue;
        }

        for (int i = 0; i <= chroma422; i++)
            h->dctf.sub8x8_dct(&dct4x4[4 * i], p_src + 8 * i * FENC_STRIDE,
                               p_dst + 8 * i * FDEC_STRIDE);
        ...
        for (int i8x8 = 0; i8x8 < (chroma422 ? 2 : 1); i8x8++) {
            if (h->mb.b_trellis) {
                for (int i4x4 = 0; i4x4 < 4; i4x4++) {
                    if (x264_quant_4x4_trellis(h, dct4x4[i8x8 * 4 + i4x4], CQM_4IC + b_inter, i_qp,
                                               DCT_CHROMA_AC, !b_inter, 1, 0)) {
        ...
        if (h->mb.b_trellis)
            nz_dc = x264_quant_chroma_dc_trellis(h, dct_dc, i_qp + 3 * chroma422, !b_inter,
                                                 CHROMA_DC + ch);
        else {
            nz_dc = 0;
            for (int i = 0; i <= chroma422; i++)
...
void x264_predict_lossless_chroma(x264_t *h, int i_mode) {
    ...
    if (i_mode == I_PRED_CHROMA_V) {
        h->mc.copy[PIXEL_8x8](h->mb.pic.p_fdec[1], FDEC_STRIDE, h->mb.pic.p_fenc[1] - FENC_STRIDE,
                              FENC_STRIDE, height);
        h->mc.copy[PIXEL_8x8](h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fenc[2] - FENC_STRIDE,
                              FENC_STRIDE, height);
        ...
        h->mc.copy[PIXEL_8x8](h->mb.pic.p_fdec[1], FDEC_STRIDE, h->mb.pic.p_fenc[1] - 1,
                              FENC_STRIDE, height);
        h->mc.copy[PIXEL_8x8](h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fenc[2] - 1,
                              FENC_STRIDE, height);
        x264_copy_column8(h->mb.pic.p_fdec[1] + 4 * FDEC_STRIDE,
                          h->mb.pic.p_fdec[1] + 4 * FDEC_STRIDE - 1);
        x264_copy_column8(h->mb.pic.p_fdec[2] + 4 * FDEC_STRIDE,
                          h->mb.pic.p_fdec[2] + 4 * FDEC_STRIDE - 1);
        if (CHROMA_FORMAT == CHROMA_422) {
            x264_copy_column8(h->mb.pic.p_fdec[1] + 12 * FDEC_STRIDE,
                              h->mb.pic.p_fdec[1] + 12 * FDEC_STRIDE - 1);
            x264_copy_column8(h->mb.pic.p_fdec[2] + 12 * FDEC_STRIDE,
                              h->mb.pic.p_fdec[2] + 12 * FDEC_STRIDE - 1);
        }
    } else {
...
void
x264_predict_lossless_8x8(x264_t *h, pixel *p_dst, int p, int idx, int i_mode, pixel edge[36]) {
    int stride = h->fenc->i_stride[p] << MB_INTERLACED;
...
static ALWAYS_INLINE void x264_macroblock_encode_internal(x264_t *h, int plane_count, int chroma) {
    ...
    if (h->mb.i_type == I_PCM) {
        ...
        for (int p = 0; p < plane_count; p++)
            h->mc.copy[PIXEL_16x16](h->mb.pic.p_fdec[p], FDEC_STRIDE, h->mb.pic.p_fenc[p],
                                    FENC_STRIDE, 16);
        if (chroma) {
            int height = 16 >> CHROMA_V_SHIFT;
            h->mc.copy[PIXEL_8x8](h->mb.pic.p_fdec[1], FDEC_STRIDE, h->mb.pic.p_fenc[1],
                                  FENC_STRIDE, height);
            h->mc.copy[PIXEL_8x8](h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fenc[2],
                                  FENC_STRIDE, height);
        }
        return;
    }
        ...
        if (!h->mb.b_skip_mc) {
            ...
            if (chroma) {
                ...
                if (mvx | mvy)
                    ...
                    h->mc.load_deinterleave_chroma_fdec(h->mb.pic.p_fdec[1],
    ...
    if (h->mb.i_type == I_16x16) {
        ...
        if (h->mb.i_skip_intra) {
            h->mc.copy[PIXEL_16x16](h->mb.pic.p_fdec[0], FDEC_STRIDE, h->mb.pic.i8x8_fdec_buf, 16,
                                    16);
            ...
            if (h->mb.i_skip_intra == 2)
                h->mc.memcpy_aligned(h->dct.luma8x8, h->mb.pic.i8x8_dct_buf,
        ...
        if (h->mb.i_skip_intra) {
            h->mc.copy[PIXEL_16x16](h->mb.pic.p_fdec[0], FDEC_STRIDE, h->mb.pic.i4x4_fdec_buf, 16,
                                    16);
        ...
        if (h->mb.b_lossless) {
            if (h->mb.b_transform_8x8)
                for (int p = 0; p < plane_count; p++)
                    for (int i8x8 = 0; i8x8 < 4; i8x8++) {
                        int x = i8x8 & 1;
                        int y = i8x8 >> 1;
                        nz = h->zigzagf.sub_8x8(h->dct.luma8x8[p * 4 + i8x8],
                                                h->mb.pic.p_fenc[p] + 8 * x + 8 * y * FENC_STRIDE,
            ...
            for (int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp) {
                ...
                for (int idx = 0; idx < 4; idx++) {
                    nz = x264_quant_8x8(h, dct8x8[idx], i_qp, ctx_cat_plane[DCT_LUMA_8x8][p], 0, p,
                                        idx);

                    if (nz) {
                        h->zigzagf.scan_8x8(h->dct.luma8x8[p * 4 + idx], dct8x8[idx]);
                        if (b_decimate) {
                            int i_decimate_8x8 = h->quantf.decimate_score64(
                                    h->dct.luma8x8[p * 4 + idx]);
                ...
                if (i_decimate_mb >= 6 || !b_decimate) {
                    ...
                    FOREACH_BIT(idx, 0, plane_cbp) {
                        ...
                        h->dctf.add8x8_idct8(
                                &h->mb.pic.p_fdec[p][8 * (idx & 1) + 8 * (idx >> 1) * FDEC_STRIDE],
                                dct8x8[idx]);
            ...
            for (int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp) {
                ...
                if (h->mb.b_noise_reduction) {
                    h->nr_count[0 + !!p * 2] += 16;
                    for (int idx = 0; idx < 16; idx++)
                        h->quantf.denoise_dct(dct4x4[idx], h->nr_residual_sum[0 + !!p * 2],
                                              h->nr_offset[0 + !!p * 2], 16);
                }
                ...
                for (int i8x8 = 0; i8x8 < 4; i8x8++) {
                    ...
                    if (h->mb.b_trellis) {
                        for (int i4x4 = 0; i4x4 < 4; i4x4++) {
                            int idx = i8x8 * 4 + i4x4;
                            if (x264_quant_4x4_trellis(h, dct4x4[idx], quant_cat, i_qp,
                                                       ctx_cat_plane[DCT_LUMA_4x4][p], 0, !!p,
                                                       p * 16 + idx)) {
                                ...
                                if (i_decimate_8x8 < 6)
                                    i_decimate_8x8 += h->quantf.decimate_score16(
                                            h->dct.luma4x4[p * 16 + idx]);
                                h->mb.cache.non_zero_count[x264_scan8[p * 16 + idx]] = 1;
                                nnz8x8 = 1;
                            }
                        }
                    } else {
                        nnz8x8 = nz = h->quantf.quant_4x4x4(&dct4x4[i8x8 * 4],
                                                            h->quant4_mf[quant_cat][i_qp],
                                                            h->quant4_bias[quant_cat][i_qp]);
                        if (nz) {
                            FOREACH_BIT(idx, i8x8 * 4, nz) {
                                h->zigzagf.scan_4x4(h->dct.luma4x4[p * 16 + idx], dct4x4[idx]);
                                h->quantf.dequant_4x4(dct4x4[idx], h->dequant4_mf[quant_cat], i_qp);
                ...
                if (i_decimate_mb < 6) {
                    ...
                    FOREACH_BIT(i8x8, 0, plane_cbp) {
                        h->dctf.add8x8_idct(&h->mb.pic.p_fdec[p][(i8x8 & 1) * 8 +
                                                                 (i8x8 >> 1) * 8 * FDEC_STRIDE],
                                            &dct4x4[i8x8 * 4]);
                    }
                }
            }
 ...
 *****************************************************************************/
static ALWAYS_INLINE int
x264_macroblock_probe_skip_internal(x264_t *h, int b_bidir, int plane_count, int chroma) {
    ALIGNED_ARRAY_64(dctcoef, dct4x4,[8],[16] );
    ...
    for (int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp) {
        ...
        for (int i8x8 = 0, i_decimate_mb = 0; i8x8 < 4; i8x8++) {
            ...
            if (h->mb.b_noise_reduction)
                for (int i4x4 = 0; i4x4 < 4; i4x4++)
                    h->quantf.denoise_dct(dct4x4[i4x4], h->nr_residual_sum[0 + !!p * 2],
                                          h->nr_offset[0 + !!p * 2], 16);

            int nz = h->quantf.quant_4x4x4(dct4x4, h->quant4_mf[quant_cat][i_qp],
                                           h->quant4_bias[quant_cat][i_qp]);
    ...
    if (chroma == CHROMA_420 || chroma == CHROMA_422) {
        ...
        for (int ch = 0; ch < 2; ch++) {
            ...
            ssd = h->pixf.ssd[chroma422 ? PIXEL_8x16 : PIXEL_8x8](p_dst, FDEC_STRIDE, p_src,
                                                                  FENC_STRIDE);
            if (ssd < thresh)
                continue;
            ...
            for (int i = 0; i <= chroma422; i++)
                if (h->quantf.quant_2x2_dc(&dct_dc[4 * i],
                                           h->quant4_mf[CQM_4PC][i_qp + 3 * chroma422][0] >> 1,
            ...
            for (int i8x8 = 0, i_decimate_mb = 0; i8x8 < (chroma422 ? 2 : 1); i8x8++) {
                int nz = h->quantf.quant_4x4x4(&dct4x4[i8x8 * 4], h->quant4_mf[CQM_4PC][i_qp],
                                               h->quant4_bias[CQM_4PC][i_qp]);
                FOREACH_BIT(idx, i8x8 * 4, nz) {
                    h->zigzagf.scan_4x4(dctscan, dct4x4[idx]);
 ...
 *****************************************************************************/
static ALWAYS_INLINE void
x264_macroblock_encode_p8x8_internal(x264_t *h, int i8, int plane_count, int chroma) {
    int b_decimate = h->mb.b_dct_decimate;
    int i_qp = h->mb.i_qp;
    int x = i8 & 1;
    int y = i8 >> 1;
    ...
    if (h->mb.b_lossless) {
        ...
        if (chroma == CHROMA_420 || chroma == CHROMA_422) {
            for (int ch = 0; ch < 2; ch++) {
                dctcoef dc;
                pixel *p_fenc =
                        h->mb.pic.p_fenc[1 + ch] + 4 * x + (chroma422 ? 8 : 4) * y * FENC_STRIDE;
                pixel *p_fdec =
                        h->mb.pic.p_fdec[1 + ch] + 4 * x + (chroma422 ? 8 : 4) * y * FDEC_STRIDE;

                for (int i4x4 = 0; i4x4 <= chroma422; i4x4++) {
                    int offset = chroma422 ? 8 * y + 2 * i4x4 + x : i8;
                    nz = h->zigzagf.sub_4x4ac(h->dct.luma4x4[16 + offset + ch * 16],
                                              p_fenc + 4 * i4x4 * FENC_STRIDE,
                                              p_fdec + 4 * i4x4 * FDEC_STRIDE, &dc);
        ...
        if (h->mb.b_transform_8x8) {
            for (int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp) {
                ...
                int nnz8x8 = x264_quant_8x8(h, dct8x8, i_qp, ctx_cat_plane[DCT_LUMA_8x8][p], 0, p,
                                            i8);
                if (nnz8x8) {
            ...
            for (int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp) {
                ...
                if (h->mb.b_trellis) {
                    for (int i4x4 = 0; i4x4 < 4; i4x4++) {
                        if (x264_quant_4x4_trellis(h, dct4x4[i4x4], quant_cat, i_qp,
                                                   ctx_cat_plane[DCT_LUMA_4x4][p], 0, !!p,
                                                   i8 * 4 + i4x4 + p * 16)) {
                            h->zigzagf.scan_4x4(h->dct.luma4x4[p * 16 + i8 * 4 + i4x4],
                                                dct4x4[i4x4]);
                            h->quantf.dequant_4x4(dct4x4[i4x4], h->dequant4_mf[quant_cat], i_qp);
                            if (i_decimate_8x8 < 4)
                                i_decimate_8x8 += h->quantf.decimate_score16(
                                        h->dct.luma4x4[p * 16 + i8 * 4 + i4x4]);
                            h->mb.cache.non_zero_count[x264_scan8[p * 16 + i8 * 4 + i4x4]] = 1;
                            nnz8x8 = 1;
                        }
                    }
                } else {
                    nnz8x8 = nz = h->quantf.quant_4x4x4(dct4x4, h->quant4_mf[quant_cat][i_qp],
                                                        h->quant4_bias[quant_cat][i_qp]);
                    if (nz) {
                        FOREACH_BIT(i4x4, 0, nz) {
                            h->zigzagf.scan_4x4(h->dct.luma4x4[p * 16 + i8 * 4 + i4x4],
        ...
        if (chroma == CHROMA_420 || chroma == CHROMA_422) {
            ...
            for (int ch = 0; ch < 2; ch++) {
                ...
                for (int i4x4 = 0; i4x4 <= chroma422; i4x4++) {
                    h->dctf.sub4x4_dct(dct4x4[i4x4], p_fenc + 4 * i4x4 * FENC_STRIDE,
                                       p_fdec + 4 * i4x4 * FDEC_STRIDE);

                    if (h->mb.b_noise_reduction)
                        h->quantf.denoise_dct(dct4x4[i4x4], h->nr_residual_sum[2], h->nr_offset[2],
                                              16);
                    dct4x4[i4x4][0] = 0;

                    if (h->mb.b_trellis)
                        nz = x264_quant_4x4_trellis(h, dct4x4[i4x4], CQM_4PC, i_qp, DCT_CHROMA_AC,
                                                    0, 1, 0);
                    else
                        nz = h->quantf.quant_4x4(dct4x4[i4x4], h->quant4_mf[CQM_4PC][i_qp],
                                                 h->quant4_bias[CQM_4PC][i_qp]);
