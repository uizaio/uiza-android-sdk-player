...
ac_energy_var( uint64_t
sum_ssd,
int shift, x264_frame_t
*frame,
int i,
int b_store
)
...
static ALWAYS_INLINE uint32_t
ac_energy_plane( x264_t
*h,
int mb_x,
int mb_y, x264_frame_t
*frame,
int i,
int b_chroma,
int b_field,
...
h->mc.
load_deinterleave_chroma_fenc( pix, frame
->plane[1] + offset, stride, height );
return
ac_energy_var( h
->pixf.var[chromapix]( pix,               FENC_STRIDE ), shift, frame, 1, b_store )
+
ac_energy_var( h
->pixf.var[chromapix]( pix+FENC_STRIDE/2, FENC_STRIDE ), shift, frame, 2, b_store );
}
else
return
ac_energy_var( h
->pixf.var[PIXEL_16x16]( frame->plane[i] + offset, stride ), 8, frame, i, b_store );
}

// Find the total AC energy of the block in all planes.
static NOINLINE uint32_t
x264_ac_energy_mb( x264_t
*h,
int mb_x,
int mb_y, x264_frame_t
*frame )
...
{
var_interlaced  +=
ac_energy_plane( h, mb_x, mb_y, frame,
1, 0, 1, 1 );
var_progressive +=
ac_energy_plane( h, mb_x, mb_y, frame,
1, 0, 0, 0 );
...
var +=
ac_energy_plane( h, mb_x, mb_y, frame,
1, 0, PARAM_INTERLACED, 1 );
var +=
ac_energy_plane( h, mb_x, mb_y, frame,
2, 0, PARAM_INTERLACED, 1 );
}
else
var +=
ac_energy_plane( h, mb_x, mb_y, frame,
1, 1, PARAM_INTERLACED, 1 );
}
...
i_mb_count;
mb_xy++ )
frame->i_inv_qscale_factor[mb_xy] =
x264_exp2fix8( frame
->f_qp_offset[mb_xy] );
}
else
{
memset( frame
->f_qp_offset, 0, h->mb.i_mb_count * sizeof(float));
memset( frame
->f_qp_offset_aq, 0, h->mb.i_mb_count * sizeof(float));
if( h->frames.b_have_lowres )
for(
int mb_xy = 0;
mb_xy < h->mb.
...
tapfilter(float *src, int pos, int max, int stride, float *coeff, int filtersize) {
    float sum = 0.f;
...
static void x264_macroblock_tree_rescale(x264_t * h, x264_ratecontrol_t * rc,
float *dst
)
{
float *input, *output;
...
int y = 0;
y<height;
y++, input += stride, output += h->mb.i_mb_width )
{
...
output[x] =
tapfilter( input, rc
->mbtree.pos[0][x], stride, 1, coeff, filtersize );
}
...
tapfilter( input, rc
->mbtree.pos[1][y], height, stride, coeff, filtersize );
}
}
...
if(
fread( rc
->mbtree.qp_buffer[rc->mbtree.qpbuf_pos], sizeof(uint16_t), rc->mbtree.src_mb_count, rc->p_mbtree_stat_file_in ) != rc->mbtree.src_mb_count )
goto
fail;
...
h->mc.
mbtree_fix8_unpack( dst, rc
->mbtree.qp_buffer[rc->mbtree.qpbuf_pos], rc->mbtree.src_mb_count );
if( rc->mbtree.rescale_enabled )
x264_macroblock_tree_rescale( h, rc, frame
->f_qp_offset );
if( h->frames.b_have_lowres )
for(
int i = 0;
...
void x264_ratecontrol_init_reconfigurable(x264_t * h,
int b_init
)
{
x264_ratecontrol_t *rc = h->rc;
...
rc->
rate_factor_constant = pow(base_cplx, 1 - rc->qcompress)
...
h->param.rc.
i_vbv_max_bitrate = h->param.rc.i_bitrate;

if( h->param.rc.

i_vbv_buffer_size<(int)(h->param.rc.i_vbv_max_bitrate / rc->fps)

)
{
h->param.rc.
i_vbv_buffer_size = h->param.rc.i_vbv_max_bitrate / rc->fps;
...
i_cpb_cnt = 1;
h->sps->vui.hrd.
b_cbr_hrd = h->param.i_nal_hrd == X264_NAL_HRD_CBR;
h->sps->vui.hrd.
i_time_offset_length = 0;
...
h->sps->vui.hrd.
i_bit_rate_scale = x264_clip3(x264_ctz(vbv_max_bitrate) - BR_SHIFT, 0, 15);
h->sps->vui.hrd.
i_bit_rate_value = vbv_max_bitrate >> (h->sps->vui.hrd.i_bit_rate_scale + BR_SHIFT);
h->sps->vui.hrd.
i_bit_rate_unscaled =
h->sps->vui.hrd.i_bit_rate_value << (h->sps->vui.hrd.i_bit_rate_scale + BR_SHIFT);
h->sps->vui.hrd.
i_cpb_size_scale = x264_clip3(x264_ctz(vbv_buffer_size) - CPB_SHIFT, 0, 15);
h->sps->vui.hrd.
i_cpb_size_value = vbv_buffer_size >> (h->sps->vui.hrd.i_cpb_size_scale + CPB_SHIFT);
h->sps->vui.hrd.
i_cpb_size_unscaled =
...
int max_cpb_output_delay = X264_MIN(
        h->param.i_keyint_max * MAX_DURATION * h->sps->vui.i_time_scale /
        h->sps->vui.i_num_units_in_tick, INT_MAX);
int max_dpb_output_delay =
        h->sps->vui.i_max_dec_frame_buffering * MAX_DURATION * h->sps->vui.i_time_scale /
        h->sps->vui.i_num_units_in_tick;
int max_delay = (int) (90000.0 * (double) h->sps->vui.hrd.i_cpb_size_unscaled /
                       h->sps->vui.hrd.i_bit_rate_unscaled + 0.5);

h->sps->vui.hrd.
i_initial_cpb_removal_delay_length = 2 + x264_clip3(32 - x264_clz(max_delay), 4, 22);
h->sps->vui.hrd.
i_cpb_removal_delay_length = x264_clip3(32 - x264_clz(max_cpb_output_delay), 4, 31);
h->sps->vui.hrd.
i_dpb_output_delay_length = x264_clip3(32 - x264_clz(max_dpb_output_delay), 4, 31);
...
if( rc->b_vbv_min_rate )
rc->
bitrate = (double) h->param.rc.i_bitrate * kilobit_size;
rc->
buffer_rate = vbv_max_bitrate / rc->fps;
rc->
vbv_max_rate = vbv_max_bitrate;
rc->
buffer_size = vbv_buffer_size;
rc->
single_frame_vbv = rc->buffer_rate * 1.1 > rc->buffer_size;
rc->
cbr_decay = 1.0 - rc->buffer_rate / rc->buffer_size
                  * 0.5 * X264_MAX(0, 1.5 - rc->buffer_rate * rc->fps / rc->bitrate);
if( h->param.rc.i_rc_method == X264_RC_CRF && h->param.rc.f_rf_constant_max )
{
rc->
rate_factor_max_increment = h->param.rc.f_rf_constant_max - h->param.rc.f_rf_constant;
if( rc->rate_factor_max_increment <= 0 )
{
x264_log( h, X264_LOG_WARNING,
"CRF max must be greater than CRF\n" );
rc->
rate_factor_max_increment = 0;
}
}
if( b_init )
{
...
h->param.rc.
f_vbv_buffer_init = x264_clip3f(h->param.rc.f_vbv_buffer_init / h->param.rc.i_vbv_buffer_size, 0,
                                1);
h->param.rc.
f_vbv_buffer_init = x264_clip3f(
        X264_MAX(h->param.rc.f_vbv_buffer_init, rc->buffer_rate / rc->buffer_size), 0, 1);
rc->
buffer_fill_final =
rc->buffer_fill_final_min =
        rc->buffer_size * h->param.rc.f_vbv_buffer_init * h->sps->vui.i_time_scale;
rc->
b_vbv = 1;
rc->
b_vbv_min_rate = !rc->b_2pass
...
int x264_ratecontrol_new(x264_t * h) {
    ...
    if (h->param.i_nal_hrd) {
        uint64_t
        denom = (uint64_t)
        h->sps->vui.hrd.i_bit_rate_unscaled * h->sps->vui.i_time_scale;
        uint64_t num = 90000;
...
#define ABR_INIT_QP (( h->param.rc.i_rc_method == X264_RC_CRF ? h->param.rc.f_rf_constant : 24 ) + QP_BD_OFFSET)
    ...
    rc->qp_constant[SLICE_TYPE_I] = x264_clip3(h->param.rc.i_qp_constant - rc->ip_offset + 0.5, 0,
                                               QP_MAX);
    rc->qp_constant[SLICE_TYPE_B] = x264_clip3(h->param.rc.i_qp_constant + rc->pb_offset + 0.5, 0,
                                               QP_MAX);
    ...
    if (h->param.rc.b_stat_read) {
        ...
        float res_factor, res_factor_bits;
        {
            ...
            *stats_in = '\0';
            ...
            if ((p = strstr(opts, "interlaced="))) {
                char *current = h->param.b_interlaced ? h->param.b_tff ? "tff" : "bff"
                                                      : h->param.b_fake_interlaced ? "fake" : "0";
                char buf[5];
            ...
            if ((p = strstr(opts, "b_adapt=")) && sscanf(p, "b_adapt=%d", &i) &&
                i >= X264_B_ADAPT_NONE && i <= X264_B_ADAPT_TRELLIS)
            ...
            if ((h->param.rc.b_mb_tree || h->param.rc.i_vbv_buffer_size) &&
                (p = strstr(opts, "rc_lookahead=")) && sscanf(p, "rc_lookahead=%d", &i))
        ...
        for (int i = 0; i < rc->num_entries; i++) {
            ...
            e += sscanf(p, " in:%*d out:%*d type:%c dur:%"
            SCNd64
            " cpbdur:%"
            SCNd64
            " q:%f aq:%f tex:%d mv:%d misc:%d imb:%d pmb:%d smb:%d d:%c",
    ...
    if (h->param.rc.b_stat_write) {
        ...
        if (h->param.rc.b_mb_tree && !h->param.rc.b_stat_read) {
            rc->psz_mbtree_stat_file_tmpname = x264_strcat_filename(h->param.rc.psz_stat_out,
                                                                    ".mbtree.temp");
            rc->psz_mbtree_stat_file_name = x264_strcat_filename(h->param.rc.psz_stat_out,
                                                                 ".mbtree");
...
static int parse_zone(x264_t * h, x264_zone_t * z,
char *p
)
{
int len = 0;
...
if( 3 <=
sscanf(p,
"%d,%d,q=%d%n", &z->i_start, &z->i_end, &z->i_qp, &len))
z->
b_force_qp = 1;
else if( 3 <=
sscanf(p,
"%d,%d,b=%f%n", &z->i_start, &z->i_end, &z->f_bitrate_factor, &len))
z->
b_force_qp = 0;
else if( 2 <=
sscanf(p,
"%d,%d%n", &z->i_start, &z->i_end, &len))
z->
b_force_qp = 0;
else
{
...
return 0;
CHECKED_MALLOC( z
->param, sizeof(x264_param_t));
memcpy( z
->param, &h->param, sizeof(x264_param_t));
z->param->
param_free = x264_free;
while((
...
static x264_zone_t *get_zone(x264_t * h,
int frame_num
)
{
for(
int i = h->rc->i_zones - 1;
i >= 0; i-- )
{
...
static void accum_p_qp_update(x264_t * h,
float qp
)
{
x264_ratecontrol_t *rc = h->rc;
...
void x264_ratecontrol_start(x264_t * h,
int i_force_qp,
int overhead
)
{
...
{
h->sh.
b_direct_spatial_mv_pred = (rce->direct_mode == 's');
h->mb.
b_direct_auto_read = (rce->direct_mode == 's' || rce->direct_mode == 't');
}
}
...
{
memset( h
->fdec->i_row_bits, 0, h->mb.i_mb_height * sizeof(int));
memset( h
->fdec->f_row_qp, 0, h->mb.i_mb_height * sizeof(float));
memset( h
->fdec->f_row_qscale, 0, h->mb.i_mb_height * sizeof(float));
rc->
row_pred = rc->row_preds[h->sh.i_type];
rc->
buffer_rate = h->fenc->i_cpb_duration * rc->vbv_max_rate * h->sps->vui.i_num_units_in_tick /
...
rc->
frame_size_maximum = 384 * BIT_DEPTH * X264_MAX(pic_size_in_mbs, fr * l->mbps) / mincr;
}
else
{
//384 * MaxMBPS * ( tr( n ) - tr( n - 1 ) ) / MinCR
rc->
frame_size_maximum = 384 * BIT_DEPTH *
                     ((double) h->fenc->i_cpb_duration * h->sps->vui.i_num_units_in_tick /
                      h->sps->vui.i_time_scale) * l->mbps / mincr;
}
}
}
...
{
rce->
new_qscale = rate_estimate_qscale(h);
...
accum_p_qp_update( h, rc
->qpm );

if( h->sh.i_type != SLICE_TYPE_B )
rc->
last_non_b_pict_type = h->sh.i_type;
}

static float predict_row_size(x264_t * h,
int y,
float qscale
)
{
...
&& (
abs(h
->fref[0][0]->i_row_satd[y] - h->fdec->i_row_satd[y]) < h->fdec->i_row_satd[y]/2))
{
...
static int row_bits_so_far(x264_t * h,
int y
)
{
int bits = 0;
for(
int i = h->i_threadslice_start;
i <=
y;
i++ )
...
static float predict_row_size_to_end(x264_t * h,
int y,
float qp
)
{
...
int x264_ratecontrol_mb(x264_t * h,
int bits
)
{
x264_ratecontrol_t *rc = h->rc;
...
h->fdec->f_row_qp[y] = rc->
qpm;
h->fdec->f_row_qscale[y] =
qscale;
...
float slice_size_planned = h->param.b_sliced_threads ? rc->slice_size_planned
...
if( h != h->thread[i] )
{
size_of_other_slices += h->thread[i]->rc->
frame_size_estimated;
size_of_other_slices_planned += h->thread[i]->rc->
slice_size_planned;
}
...
float b_max =
        b1 + ((rc->buffer_fill - rc->buffer_size + rc->buffer_rate) * 0.90f - b1) * trust_coeff;
rc->qpm -=
step_size;
...
h->rc->
frame_size_estimated = b1 - size_of_other_slices;
...
rc->
qpm = x264_clip3f((prev_row_qp + rc->qpm) * 0.5f, prev_row_qp + 1.0f, qp_max);
rc->
qpa_rc = rc->qpa_rc_prev;
rc->
qpa_aq = rc->qpa_aq_prev;
...
{
h->rc->
frame_size_estimated = bits_so_far;

/* Last-ditch attempt: if the last row of the frame underflowed the VBV,
         * try again. */
if( rc->qpm < qp_max && can_reencode_row
&& (h->rc->frame_size_estimated + size_of_other_slices >
X264_MIN( rc
->frame_size_maximum, rc->buffer_fill )))
{
rc->
qpm = qp_max;
rc->
...
rc->
qpa_rc_prev = rc->qpa_rc;
rc->
qpa_aq_prev = rc->qpa_aq;
...
int x264_ratecontrol_mb_qp(x264_t * h) {
    ...
    if (h->param.rc.i_aq_mode) {
        ...
        float qp_offset = h->fdec->b_kept_as_ref ? h->fenc->f_qp_offset[h->mb.i_mb_xy]
...
int x264_ratecontrol_slice_type(x264_t * h,
int frame_num
)
{
x264_ratecontrol_t *rc = h->rc;
...
h->param.rc.
i_qp_constant = (h->stat.i_frame_count[SLICE_TYPE_P] == 0) ? 24 + QP_BD_OFFSET
                                                           : 1 + h->stat.f_frame_qp[SLICE_TYPE_P] /
                                                                 h->stat.i_frame_count[SLICE_TYPE_P];
rc->qp_constant[SLICE_TYPE_P] =
x264_clip3( h
->param.rc.i_qp_constant, 0, QP_MAX );
rc->qp_constant[SLICE_TYPE_I] = x264_clip3((
int)(
qscale2qp( qp2qscale(h->param.rc.i_qp_constant)
/
fabs( h
->param.rc.f_ip_factor )) + 0.5 ), 0, QP_MAX );
rc->qp_constant[SLICE_TYPE_B] = x264_clip3((
int)(
qscale2qp( qp2qscale(h->param.rc.i_qp_constant)
*
fabs( h
->param.rc.f_pb_factor )) + 0.5 ), 0, QP_MAX );
...
void x264_ratecontrol_set_weights(x264_t * h, x264_frame_t * frm) {
    ...
    if (rce->i_weight_denom[0] >= 0)
        SET_WEIGHT(frm->weight[0][0], 1, rce->weight[0][0], rce->i_weight_denom[0],
                   rce->weight[0][1]);

    if (rce->i_weight_denom[1] >= 0) {
        SET_WEIGHT(frm->weight[0][1], 1, rce->weight[1][0], rce->i_weight_denom[1],
                   rce->weight[1][1]);
        SET_WEIGHT(frm->weight[0][2], 1, rce->weight[2][0], rce->i_weight_denom[1],
                   rce->weight[2][1]);
    }
}

/* After encoding one frame, save stats and update ratecontrol state */
int x264_ratecontrol_end(x264_t * h,
int bits,
int *filler
)
{
...
h->stat.frame.
i_mb_count_skip = mbs[P_SKIP] + mbs[B_SKIP];
h->stat.frame.
i_mb_count_i = mbs[I_16x16] + mbs[I_8x8] + mbs[I_4x4];
h->stat.frame.
i_mb_count_p = mbs[P_L0] + mbs[P_8x8];
for(
int i = B_DIRECT;
i<B_8x8;
i++ )
...
h->fdec->
f_qp_avg_rc = rc->qpa_rc /= h->mb.i_mb_count;
h->fdec->
f_qp_avg_aq = (float) rc->qpa_aq / h->mb.i_mb_count;
h->fdec->
f_crf_avg = h->param.rc.f_rf_constant + h->fdec->f_qp_avg_rc - rc->qp_novbv;
...
h->mc.
mbtree_fix8_pack( rc
->mbtree.qp_buffer[0], h->fenc->f_qp_offset, h->mb.i_mb_count );
if( fwrite( &i_type, 1, 1, rc->p_mbtree_stat_file_out ) < 1 )
goto
fail;
if(
fwrite( rc
->mbtree.qp_buffer[0], sizeof(uint16_t), h->mb.i_mb_count, rc->p_mbtree_stat_file_out ) < h->mb.i_mb_count )
goto
fail;
}
}
...
bits *qp2qscale(rc->qpa_rc)
/ (rc->
last_rceq *fabs(h->param.rc.f_pb_factor)
);
}
rc->cplxr_sum *= rc->
cbr_decay;
rc->wanted_bits_window += h->fenc->
f_duration *rc
->
bitrate;
rc->wanted_bits_window *= rc->
cbr_decay;
}
...
rc->
initial_cpb_removal_delay = h->initial_cpb_removal_delay;
rc->
initial_cpb_removal_delay_offset = h->initial_cpb_removal_delay_offset;
h->fenc->hrd_timing.
cpb_removal_time = rc->nrt_first_access_unit = (double) rc->initial_cpb_removal_delay / 90000;
}
else
{
h->fenc->hrd_timing.
cpb_removal_time =
rc->nrt_first_access_unit + (double) (h->fenc->i_cpb_delay - h->i_cpb_delay_pir_offset) *
                            h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;

if( h->fenc->b_keyframe )
{
rc->
nrt_first_access_unit = h->fenc->hrd_timing.cpb_removal_time;
rc->
initial_cpb_removal_delay = h->initial_cpb_removal_delay;
rc->
initial_cpb_removal_delay_offset = h->initial_cpb_removal_delay_offset;
...
double cpb_earliest_arrival_time =
        h->fenc->hrd_timing.cpb_removal_time - (double) rc->initial_cpb_removal_delay / 90000;
if( !h->fenc->b_keyframe )
...
h->fenc->hrd_timing.
cpb_initial_arrival_time = X264_MAX(rc->previous_cpb_final_arrival_time, cpb_earliest_arrival_time);
}
...
cpb_final_arrival_time = rc->previous_cpb_final_arrival_time =
                                 ...
                                 (double) (bits + filler_bits) /
                                 h->sps->vui.hrd.i_bit_rate_unscaled;

h->fenc->hrd_timing.
dpb_output_time =
(double) h->fenc->i_dpb_output_delay * h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale +
 ...
 */
static double get_qscale(x264_t * h, ratecontrol_entry_t * rce,
double rate_factor,
int frame_num
)
{
...
q = pow(BASE_FRAME_DURATION / CLIP_DURATION(rce->i_duration * timescale),
        1 - h->param.rc.f_qcompress);
}
else
...
static double get_diff_limited_q(x264_t * h, ratecontrol_entry_t * rce,
double q,
int frame_num
)
{
...
rcc->
last_non_b_pict_type = pict_type;
if( pict_type == SLICE_TYPE_I )
{
rcc->
last_accum_p_norm = rcc->accum_p_norm;
rcc->
accum_p_norm = 0;
rcc->
accum_p_qp = 0;
...
rcc->
accum_p_qp = mask * (qscale2qp(q) + rcc->accum_p_qp);
rcc->
accum_p_norm = mask * (1 + rcc->accum_p_norm);
}
...
static int update_vbv(x264_t * h,
int bits
)
{
int filler = 0;
...
update_predictor( &rct->pred[h->sh.i_type],
qp2qscale( rcc
->qpa_rc ), rcc->last_satd, bits );

if( !rcc->b_vbv )
return
filler;

uint64_t buffer_diff = (uint64_t)
bits *h
->sps->vui.
i_time_scale;
...
buffer_diff = (uint64_t)
bitrate *h
->sps->vui.
i_num_units_in_tick *h
->fenc->
i_cpb_duration;
rct->buffer_fill_final +=
buffer_diff;
...
bits = h->param.i_avcintra_class ? filler * 8 :
       X264_MAX((FILLER_OVERHEAD - h->param.b_annexb), filler) * 8;
buffer_diff = (uint64_t)
bits *h
->sps->vui.
i_time_scale;
...
{
rct->
buffer_fill_final = X264_MIN(rct->buffer_fill_final, buffer_size);
rct->
buffer_fill_final_min = X264_MIN(rct->buffer_fill_final_min, buffer_size);
}
}
...
void x264_hrd_fullness(x264_t * h) {
    ...
    uint64_t
    denom = (uint64_t)
    h->sps->vui.hrd.i_bit_rate_unscaled * h->sps->vui.i_time_scale / rct->hrd_multiply_denom;
    uint64_t cpb_state = rct->buffer_fill_final;
    uint64_t
    cpb_size = (uint64_t)
    h->sps->vui.hrd.i_cpb_size_unscaled * h->sps->vui.i_time_scale;
    ...
    if (rct->buffer_fill_final<0 || rct->buffer_fill_final>(int64_t)cpb_size )
    {
        x264_log(h, X264_LOG_WARNING, "CPB %s: %.0f bits in a %.0f-bit buffer\n",
                 ...
                 (double) rct->buffer_fill_final / h->sps->vui.i_time_scale,
                 (double) cpb_size / h->sps->vui.i_time_scale);
    }

    h->initial_cpb_removal_delay = (multiply_factor * cpb_state) / denom;
    h->initial_cpb_removal_delay_offset =
            (multiply_factor * cpb_size) / denom - h->initial_cpb_removal_delay;
...
x264_ratecontrol_t *rcc = h->rc;
rcc->
buffer_fill = h->thread[0]->rc->buffer_fill_final_min / h->sps->vui.i_time_scale;
...
bits;
rcc->
buffer_fill = X264_MAX(rcc->buffer_fill, 0);
rcc->buffer_fill += t->rc->
buffer_rate;
rcc->
buffer_fill = X264_MIN(rcc->buffer_fill, rcc->buffer_size);
}
}
rcc->
buffer_fill = X264_MIN(rcc->buffer_fill, rcc->buffer_size);
...
static double clip_qscale(x264_t * h,
int pict_type,
double q
)
{
...
for(
int iterations = 0;
iterations < 1000 && terminate != 3; iterations++ )
{
double frame_q[3];
...
frame_q[0] = h->sh.i_type == SLICE_TYPE_I ?
q *h
->param.rc.
f_ip_factor : q;
frame_q[1] = frame_q[0] * h->param.rc.
f_pb_factor;
frame_q[2] = frame_q[0] / h->param.rc.
f_ip_factor;

/* Loop over the planned future frames. */
for(
int j = 0;
buffer_fill_cur >= 0 && buffer_fill_cur <= rcc->
buffer_size;
j++ )
{
total_duration +=
...
target_fill = X264_MIN(rcc->buffer_fill + total_duration * rcc->vbv_max_rate * 0.5,
                       rcc->buffer_size * 0.5);
...
target_fill = x264_clip3f(rcc->buffer_fill - total_duration * rcc->vbv_max_rate * 0.5,
                          rcc->buffer_size * 0.8, rcc->buffer_size);
...
double max_fill_factor =
        h->param.rc.i_vbv_buffer_size >= 5 * h->param.rc.i_vbv_max_bitrate / rcc->fps ? 2 : 1;
...
q *=
X264_MAX( pbbits
/ space, bits / (0.5 * rcc->buffer_size));
}
q = X264_MAX(q0 / 2, q);
}
...
static float rate_estimate_qscale(x264_t * h) {
    ...
    if (pict_type == SLICE_TYPE_B) {
        ...
        if (rcc->b_2pass)
            ...
            rcc->frame_size_planned = predict_size(rcc->pred_b_from_p, q,
                                                   h->fref[1][h->i_ref[1] - 1]->i_satd);
        ...
        } else /* 1pass ABR */
        {
            ...
            rcc->short_term_cplxsum +=
                    rcc->last_satd / (CLIP_DURATION(h->fenc->f_duration) / BASE_FRAME_DURATION);
            ...
            if (h->param.rc.i_rc_method == X264_RC_CRF) {
                ...
                if (!rcc->b_vbv_min_rate && rcc->last_satd) {
                    ...
                    if (h->param.b_vfr_input && i_frame_done > 0)
                        time_done =
                                ((double) (h->fenc->i_reordered_pts - h->i_reordered_pts_delay)) *
                                h->param.i_timebase_num / h->param.i_timebase_den;
                    wanted_bits = time_done * rcc->bitrate;
                    if (wanted_bits > 0) {
                        abr_buffer *= X264_MAX(1, sqrt(time_done));
                        overflow = x264_clip3f(1.0 + (predicted_bits - wanted_bits) / abr_buffer,
                                               .5, 2);
                        q *= overflow;
                    }
                }
...
void x264_threads_distribute_ratecontrol(x264_t * h) {
    ...
    for (int i = 0; i < h->param.i_threads; i++) {
        ...
        if (rc->b_vbv && rc->frame_size_planned) {
            ...
            t->rc->slice_size_planned = predict_size(&rc->pred[h->sh.i_type + (i + 1) * 5], qscale,
                                                     size);
        } else
    ...
    if (rc->b_vbv && rc->frame_size_planned) {
        ...
        if (rc->single_frame_vbv) {
            ...
            for (int i = 0; i < h->param.i_threads; i++) {
                x264_t * t = h->thread[i];
                float max_frame_error = x264_clip3f(
                        1.0 / (t->i_threadslice_end - t->i_threadslice_start), 0.05, 0.25);
...
void x264_threads_merge_ratecontrol(x264_t * h) {
    ...
    for (int i = 0; i < h->param.i_threads; i++) {
        ...
        if (h->param.rc.i_vbv_buffer_size) {
            ...
            update_predictor(&rc->pred[h->sh.i_type + (i + 1) * 5],
                             qp2qscale(rct->qpa_rc / mb_count), size, bits);
        }
        if (!i)
            continue;
...
static int find_underflow(x264_t * h,
double *fills,
int *t0,
int *t1,
int over
...
i++ )
{
fill += (rcc->entry_out[i]->
i_cpb_duration *rcc
->
vbv_max_rate *h
->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale -
qscale2bits( rcc
->entry_out[i], rcc->entry_out[i]->new_qscale )) *
parity;
...
static int fix_underflow(x264_t * h,
int t0,
int t1,
double adjustment,
double qscale_min,
...
int iterations = 0;
int adj_min, adj_max;
CHECKED_MALLOC( fills, (rcc
->num_entries+1)*sizeof(double));
...
i++ )
rcc->entry_out[i]->
expected_vbv = rcc->buffer_size - fills[i];
    ...
    for (int i = 0; i < rcc->num_entries; i++) {
        ...
        for (int j = 1; j < cplxblur * 2 && j < rcc->num_entries - i; j++) {
            ...
            double frame_duration =
                    CLIP_DURATION(rcj->i_duration * timescale) / BASE_FRAME_DURATION;
