...
static ALWAYS_INLINE int cached_satd(x264_t * h,
int size,
int x,
int y
)
...
static inline int ssd_plane(x264_t * h,
int size,
int p,
int x,
int y
...
return h->pixf.ssd[size](fenc, FENC_STRIDE, fdec, FDEC_STRIDE) +
satd;
}
...
static inline int ssd_mb(x264_t * h) {
    ...
    chroma_ssd = ((uint64_t)
    chroma_ssd * h->mb.i_chroma_lambda2_offset + 128) >> 8;
    return ssd_plane(h, PIXEL_16x16, 0, 0, 0) + chroma_ssd;
}

static int x264_rd_cost_mb(x264_t * h,
int i_lambda2
)
{
...
COPY_CABAC;
x264_macroblock_size_cabac( h,
&cabac_tmp );
i_bits = ((uint64_t)
cabac_tmp.
f8_bits_encoded *i_lambda2
+ 32768 ) >> 16;
}
else
{
...
i_bits = ((uint64_t)
h->out.bs.
i_bits_encoded *i_lambda2
+ 128 ) >> 8;
}

h->mb.
b_transform_8x8 = b_transform_bak;
h->mb.
i_type = type_bak;
...
static uint64_t x264_rd_cost_subpart(x264_t * h,
int i_lambda2,
int i4,
int i_pixel
)
...
chromassd = ((uint64_t)
chromassd *h
->mb.i_chroma_lambda2_offset + 128) >> 8;
i_ssd +=
chromassd;
}
...
x264_cabac_t cabac_tmp;
COPY_CABAC;
x264_subpartition_size_cabac( h,
&cabac_tmp, i4, i_pixel );
i_bits = ((uint64_t)
cabac_tmp.
f8_bits_encoded *i_lambda2
+ 128 ) >> 8;
...
uint64_t x264_rd_cost_part(x264_t * h,
int i_lambda2,
int i4,
int i_pixel
)
...
return
x264_rd_cost_subpart( h, i_lambda2, i4, i_pixel
);
...
x264_cabac_t cabac_tmp;
COPY_CABAC;
x264_partition_size_cabac( h,
&cabac_tmp, i8, i_pixel );
i_bits = ((uint64_t)
cabac_tmp.
f8_bits_encoded *i_lambda2
+ 128 ) >> 8;
...
i_bits = (uint64_t)
x264_partition_size_cavlc( h, i8, i_pixel
) *
i_lambda2;
...
static uint64_t x264_rd_cost_i8x8(x264_t * h,
int i_lambda2,
int i8,
int i_mode, pixel
edge[4][32] )
...
x264_cabac_t cabac_tmp;
COPY_CABAC;
x264_partition_i8x8_size_cabac( h,
&cabac_tmp, i8, i_mode );
i_bits = ((uint64_t)
cabac_tmp.
f8_bits_encoded *i_lambda2
...
i_bits = (uint64_t)
x264_partition_i8x8_size_cavlc( h, i8, i_mode
) *
i_lambda2;
...
static uint64_t x264_rd_cost_i4x4(x264_t * h,
int i_lambda2,
int i4,
int i_mode
)
...
x264_cabac_t cabac_tmp;
COPY_CABAC;
x264_partition_i4x4_size_cabac( h,
&cabac_tmp, i4, i_mode );
i_bits = ((uint64_t)
cabac_tmp.
f8_bits_encoded *i_lambda2
...
i_bits = (uint64_t)
x264_partition_i4x4_size_cavlc( h, i4, i_mode
) *
i_lambda2;
...
static uint64_t x264_rd_cost_chroma(x264_t * h,
int i_lambda2,
int i_mode,
int b_dct
)
...
h->mb.
i_chroma_pred_mode = i_mode;

if( h->param.b_cabac )
{
x264_cabac_t cabac_tmp;
COPY_CABAC;
x264_chroma_size_cabac( h,
&cabac_tmp );
i_bits = ((uint64_t)
cabac_tmp.
f8_bits_encoded *i_lambda2
...
static NOINLINE
int trellis_dc_shortcut(int sign_coef, int quant_coef, int unquant_mf, int coef_weight, int lambda2,
                        uint8_t *cabac_state, int cost_sig) {
    ...
    for (int abs_level = q - 1; abs_level <= q; abs_level++) {
        ...
        if (abs_level) {
            ...
            score += (uint64_t)
            f8_bits * lambda2 >> (CABAC_SIZE_BITS - LAMBDA_BITS);
        }
...
int trellis_coef(int j, int const_level, int abs_level, int prefix, int suffix_cost,
    ...
    uint8_t level1_state = (j >= 3) ? nodes_prev[j].cabac_state[level1_ctx >> 2]
                                    : level_state[level1_ctx];
    f8_bits += x264_cabac_entropy[level1_state ^ (const_level > 1)];
    uint8_t levelgt1_state;
    if (const_level > 1) {
        levelgt1_state =
                j >= 6 ? nodes_prev[j].cabac_state[levelgt1_ctx - 6] : level_state[levelgt1_ctx];
    ...
    if (score < nodes_cur[node_ctx].score) {
        ...
        if (j >= 3) // skip the transition if we're not going to reuse the context
            nodes_cur[node_ctx].cabac_state[level1_ctx >> 2] = x264_cabac_transition[level1_state][
                    const_level > 1];
        if (const_level > 1 && node_ctx == 7)
            nodes_cur[node_ctx].cabac_state[levelgt1_ctx -
                                            6] = x264_cabac_transition_unary[prefix][levelgt1_state];
...
static NOINLINE
int trellis_coef0_0(uint64_t
ssd0,
trellis_node_t *nodes_cur, trellis_node_t
*nodes_prev,
trellis_level_t *level_tree,
int levels_used
)
{
nodes_cur[0].
score = nodes_prev[0].score + ssd0;
nodes_cur[0].
level_idx = nodes_prev[0].level_idx;
for(
int j = 1;
j < 4 && (int64_t)nodes_prev[j].score >= 0; j++ )
{
nodes_cur[j].
score = nodes_prev[j].score;
if( j >= 3 )
M32(nodes_cur[j]
.cabac_state) =
M32(nodes_prev[j]
...
int quant_trellis_cabac(x264_t * h, dctcoef * dct,
                        udctcoef * quant_mf, udctcoef * quant_bias, const
int *unquant_mf,
...
{
ALIGNED_ARRAY_64( dctcoef, orig_coefs,
[64] );
ALIGNED_ARRAY_64( dctcoef, quant_coefs,
[64] );
...
if( num_coefs == 16 )
{
memcpy( orig_coefs, dct,
sizeof(dctcoef)*16 );
if( !h->quantf.
quant_4x4_dc( dct, quant_mf[0]
>> 1, quant_bias[0] << 1 ))
return 0;
h->zigzagf.
scan_4x4( quant_coefs, dct
);
}
else
{
memcpy( orig_coefs, dct,
sizeof(dctcoef)*num_coefs );
...
{
h->mc.
memcpy_aligned( orig_coefs, dct,
sizeof(dctcoef)*64 );
if( !h->quantf.
quant_8x8( dct, quant_mf, quant_bias
))
return 0;
h->zigzagf.
scan_8x8( quant_coefs, dct
);
}
...
sizeof(dctcoef)*16 );
if( !h->quantf.
quant_4x4( dct, quant_mf, quant_bias
))
return 0;
h->zigzagf.
scan_4x4( quant_coefs, dct
...
dct[0] =
trellis_dc_shortcut( orig_coefs[0], quant_coefs[0], unquant_mf[0], coef_weight2[0], lambda2, cabac_state, cost_sig
);
return !!dct[0];
}
...
j < 4; j++ )
nodes_cur[j].
score = TRELLIS_SCORE_MAX;
nodes_cur[0].
score = TRELLIS_SCORE_BIAS;
nodes_cur[0].
level_idx = 0;
level_tree[0].
abs_level = 0;
...
ALIGNED_4( uint8_t
level_state[16] );
memcpy( level_state, cabac_state,
10 );
...
int nC = b_chroma && dc ? 3 + (num_coefs >> 2)
                        : ct_index[x264_mb_predict_non_zero_code(h,
                                                                 !b_chroma && dc ? (idx - LUMA_DC) *
                                                                                   16 : idx)];
...
quant_coefs[1][i] = quant_coefs[0][i] =
sign *nearest_quant;
...
int unquant1 = (((dc ? unquant_mf[0] << 1 : unquant_mf[zigzag[j]]) * (nearest_quant - 0) + 128)
        >> 8);
int unquant0 = (((dc ? unquant_mf[0] << 1 : unquant_mf[zigzag[j]]) * (nearest_quant - 1) + 128)
        >> 8);
...
-
d1 *d1
) * (dc?256:coef_weight2[zigzag[j]]);

/* Psy trellis: bias in favor of higher AC coefficients in the reconstructed frame. */
if( h->mb.i_psy_trellis && j && !dc && !b_chroma )
{
int orig_coef = b_8x8 ? h->mb.pic.fenc_dct8[idx >> 2][zigzag[j]]
                      : h->mb.pic.fenc_dct4[idx][zigzag[j]];
...
delta_distortion[i] += (psy_value0 - psy_value1) *
psy_weight;
}
...
x264_cavlc_block_residual_internal( h, ctx_block_cat, coefs
+ b_ac, nC );
...
x264_cavlc_block_residual_internal( h, ctx_block_cat, coefs
+ b_ac, nC );
cur_score += (int64_t)h->out.bs.
i_bits_encoded *lambda2;
...
int x264_quant_luma_dc_trellis(x264_t * h, dctcoef * dct,
int i_quant_cat,
int i_qp,
int ctx_block_cat,
int b_intra,
...
if( h->param.b_cabac )
return
quant_trellis_cabac( h, dct,
        h
->quant4_mf[i_quant_cat][i_qp], h->quant4_bias0[i_quant_cat][i_qp],
...
return
quant_trellis_cavlc( h, dct,
        h
->quant4_mf[i_quant_cat][i_qp], h->unquant4_mf[i_quant_cat][i_qp], x264_zigzag_scan4[MB_INTERLACED],
...
int x264_quant_chroma_dc_trellis(x264_t * h, dctcoef * dct,
int i_qp,
int b_intra,
int idx
)
...
quant_trellis_cabac( h, dct,
        h
->quant4_mf[quant_cat][i_qp], h->quant4_bias0[quant_cat][i_qp],
...
quant_trellis_cavlc( h, dct,
        h
->quant4_mf[quant_cat][i_qp], h->unquant4_mf[quant_cat][i_qp], zigzag,
...
quant_trellis_cabac( h, dct,
        h
->quant8_mf[i_quant_cat][i_qp], h->quant8_bias0[i_quant_cat][i_qp],
...
int nz = quant_trellis_cavlc(h, dct,
                             ...
                             DCT_LUMA_4x4, h->mb.i_trellis_lambda2[b_chroma][b_intra], 0, b_chroma,
                             0, 16, idx * 4 + i, 1);
/* Set up nonzero count for future calls */
h->mb.cache.non_zero_count[x264_scan8[idx*4+i]] =
nz;
nzaccum |=
nz;
}
