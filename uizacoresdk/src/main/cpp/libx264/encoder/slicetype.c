...
static NOINLINE pixel
*
x264_weight_cost_init_luma( x264_t
*h,
x264_frame_t *fenc, x264_frame_t
*ref,
pixel *dest
)
{
...
h->mc.
mc_luma( p
+x, i_stride, ref->lowres, i_stride,
...
x264_weight_cost_init_chroma(x264_t * h, x264_frame_t * fenc, x264_frame_t * ref, pixel * dstu,
                             pixel * dstv) {
    int ref0_distance = fenc->i_frame - ref->i_frame - 1;
    ...
    if (fenc->lowres_mvs[0][ref0_distance][0][0] != 0x7FFF) {
        ...
        for (int y = 0, mb_xy = 0, pel_offset_y = 0;
             y < i_lines; y += height, pel_offset_y = y * i_stride)
            for (int x = 0, pel_offset_x = 0; x < i_width; x += 8, mb_xy++, pel_offset_x += 8) {
                ...
                h->mc.mc_chroma(pixu, pixv, i_stride, src1, i_stride, mvx, 2 * mvy >> v_shift, 8,
                                height);
            }
    } else
        h->mc.plane_copy_deinterleave(dstu, i_stride, dstv, i_stride, ref->plane[1], i_stride, cw,
                                      ch);
    h->mc.plane_copy_deinterleave(dstu + i_offset, i_stride, dstv + i_offset, i_stride,
                                  fenc->plane[1], i_stride, cw, ch);
    x264_emms();
}

static NOINLINE pixel
*
x264_weight_cost_init_chroma444( x264_t
*h,
x264_frame_t *fenc, x264_frame_t
*ref,
pixel *dst,
...
for(
int y = 0, mb_xy = 0, pel_offset_y = 0;
y<i_lines;
y += 16,
pel_offset_y = y * i_stride
)
for(
int x = 0, pel_offset_x = 0;
x<i_width;
x += 16, mb_xy++, pel_offset_x += 16 )
{
...
h->mc.
copy_16x16_unaligned( pix, i_stride, src
+mvx+
mvy *i_stride, i_stride,
16 );
}
...
numslices =
(h->mb.i_mb_width * h->mb.i_mb_height + h->param.i_slice_max_mbs - 1) / h->param.i_slice_max_mbs;
else
numslices = 1;
...
x264_weight_cost_luma(x264_t * h, x264_frame_t * fenc, pixel * src, x264_weight_t * w) {
    unsigned int cost = 0;
    int i_stride = fenc->i_stride_lowres;
    int i_lines = fenc->i_lines_lowres;
    int i_width = fenc->i_width_lowres;
    ...
    if (w) {
        ...
        for (int y = 0; y < i_lines; y += 8, pixoff = y * i_stride)
            for (int x = 0; x < i_width; x += 8, i_mb++, pixoff += 8) {
                int cmp = h->pixf.mbcmp[PIXEL_8x8](&src[pixoff], i_stride, &fenc_plane[pixoff],
                                                   i_stride);
                cost += X264_MIN(cmp, fenc->i_intra_cost[i_mb]);
            }
    x264_emms();
...
static NOINLINE unsigned int
x264_weight_cost_chroma(x264_t * h, x264_frame_t * fenc, pixel * ref, x264_weight_t * w) {
    unsigned int cost = 0;
    int i_stride = fenc->i_stride[1];
    int i_lines = fenc->i_lines[1];
    int i_width = fenc->i_width[1];
...
static NOINLINE unsigned int x264_weight_cost_chroma444(x264_t * h, x264_frame_t * fenc,
                                                        pixel * ref, x264_weight_t * w,
int p
)
{
unsigned int cost = 0;
...
cost +=
x264_weight_slice_header_cost( h, w,
1 );
}
else
for(
...
void x264_weights_analyse(x264_t * h, x264_frame_t * fenc, x264_frame_t * ref,
int b_lookahead
)
{
...
for(
int plane = 0;
plane <= 2 && !( plane && ( !weights[0].weightfn || b_lookahead )); plane++ )
{
...
//early termination
if(
fabsf( ref_mean[plane]
- fenc_mean[plane] ) < 0.5f && fabsf( 1.f - guess_scale[plane] ) < epsilon )
{
SET_WEIGHT( weights[plane],
0, 1, 0, 0 );
continue;
}
...
{
weights[plane].
i_denom = chroma_denom;
weights[plane].
i_scale = x264_clip3(round(guess_scale[plane] * (1 << chroma_denom)), 0, 255);
if( weights[plane].i_scale > 127 )
{
weights[1].
weightfn = weights[2].weightfn = NULL;
break;
}
}
else
x264_weight_get_h264( round(guess_scale[plane] * 128),
0, &weights[plane] );
...
x264_lowres_context_init( h,
&a );
x264_slicetype_frame_cost( h,
&a, &fenc, 0, 0, 0 );
}
...
int cur_offset =
        fenc_mean[plane] - ref_mean[plane] * cur_scale / (1 << mindenom) + 0.5f * b_lookahead;
...
for(
int i_off = start_offset;
i_off <=
end_offset;
i_off++ )
{
SET_WEIGHT( weights[plane],
1, cur_scale, mindenom, i_off );
unsigned int s;
if( plane )
{
if( CHROMA444 )
...
SET_WEIGHT( weights[plane],
1, minscale, mindenom, minoff );

if( h->param.analyse.i_weighted_pred == X264_WEIGHTP_FAKE && weights[0].weightfn && !plane )
fenc->f_weighted_cost_delta[i_delta_index] = (float)minscore /
origscore;
}
...
x264_weight_scale_plane( h, dst, ref
->i_stride_lowres, src, ref->i_stride_lowres,
width, height, &weights[0] );
fenc->weighted[0] = h->mb.p_weight_buf[0] + PADH + ref->
i_stride_lowres *PADV;
}
}
...
m[0].
p_fref_w = m[0].p_fref[0];
if( w[0].weightfn )
LOAD_WPELS_LUMA(m[0].p_fref_w, fenc->weighted[0]);

if( b_bidir )
{
ALIGNED_ARRAY_8( int16_t, dmv,
[2],[2] );
...
i_ref = 0;
m[1].
weight = x264_weight_none;
LOAD_HPELS_LUMA(m[1].p_fref, fref1->lowres);
m[1].
p_fref_w = m[1].p_fref[0];
...
{
m[l].
cost = h->pixf.mbcmp[PIXEL_8x8](m[l].p_fenc[0], FENC_STRIDE, m[l].p_fref[0], m[l].i_stride[0]);
if( m[l].cost < 64 )
{
M32( m[l]
.mv ) = 0;
...
memcpy( pix
-FDEC_STRIDE, src-i_stride, 16 * sizeof(pixel));
for(
int i = -1;
i < 8; i++ )
M32( &pix[
...
h->
predict_8x8_filter( pix, edge, ALL_NEIGHBORS, ALL_NEIGHBORS
);
for(
int i = 3;
i < 9; i++ )
...
output_inter[ROW_SATD] +=
i_bcost_aq;
if( b_frame_score_mb )
{
/* Don't use AQ-weighted costs for slicetype decision, only for ratecontrol. */
output_inter[COST_EST] +=
i_bcost;
output_inter[COST_EST_AQ] +=
i_bcost_aq;
}
}

fenc->lowres_costs[b-p0][p1-b][i_mb_xy] =
X264_MIN( i_bcost, LOWRES_COST_MASK
...
static void x264_slicetype_slice_cost(x264_slicetype_slice_t *s) {
    ...
    int do_edges =
            h->param.rc.b_mb_tree || h->param.rc.i_vbv_buffer_size || h->mb.i_mb_width <= 2 ||
            h->mb.i_mb_height <= 2;
...
x264_weights_analyse( h, fenc, frames[p0],
1 );
w = fenc->weight[0];
}
...
t->mb.
i_me_method = h->mb.i_me_method;
t->mb.
i_subpel_refine = h->mb.i_subpel_refine;
t->mb.
b_chroma_me = h->mb.b_chroma_me;

s[i] = (x264_slicetype_slice_t){
t, a, frames, p0, p1, b, dist_scale_factor, do_search, w,
output_inter[i], output_intra[i] };

t->
i_threadslice_start = ((h->mb.i_mb_height * i + h->param.i_lookahead_threads / 2) /
                       h->param.i_lookahead_threads);
t->
i_threadslice_end = ((h->mb.i_mb_height * (i + 1) + h->param.i_lookahead_threads / 2) /
                     h->param.i_lookahead_threads);

int thread_height = t->i_threadslice_end - t->i_threadslice_start;
int thread_output_size = thread_height + NUM_INTS;
memset( output_inter[i],
0, thread_output_size * sizeof(int));
memset( output_intra[i],
0, thread_output_size * sizeof(int));
output_inter[i][NUM_ROWS] = output_intra[i][NUM_ROWS] =
thread_height;

output_inter[i+1] = output_inter[i] + thread_output_size + PAD_SIZE;
output_intra[i+1] = output_intra[i] + thread_output_size + PAD_SIZE;

x264_threadpool_run( h
->lookaheadpool, (void*)x264_slicetype_slice_cost, &s[i] );
}
for(
int i = 0;
i < h->param.
i_lookahead_threads;
i++ )
x264_threadpool_wait( h
->lookaheadpool, &s[i] );
}
else
{
h->
i_threadslice_start = 0;
h->
i_threadslice_end = h->mb.i_mb_height;
memset( output_inter[0],
0, (output_buf_size - PAD_SIZE) * sizeof(int));
memset( output_intra[0],
0, (output_buf_size - PAD_SIZE) * sizeof(int));
output_inter[0][NUM_ROWS] = output_intra[0][NUM_ROWS] = h->mb.
i_mb_height;
...
memcpy( row_satd_inter, output_inter[i]
+ NUM_INTS, row_count * sizeof(int));
if( !fenc->b_intra_calculated )
memcpy( row_satd_intra, output_intra[i]
+ NUM_INTS, row_count * sizeof(int));
row_satd_inter +=
row_count;
row_satd_intra +=
...
fenc->
b_intra_calculated = 1;

fenc->i_cost_est[b-p0][p1-b] =
i_score;
...
static int x264_slicetype_frame_cost_recalculate(x264_t * h, x264_frame_t * *frames,
int p0,
int p1,
int b
)
...
float *qp_offset = IS_X264_TYPE_B(frames[b]->i_type) ? frames[b]->f_qp_offset_aq
                                                     : frames[b]->f_qp_offset;

x264_emms();

for( h->mb.
i_mb_y = h->mb.i_mb_height - 1;
h->mb.i_mb_y >= 0; h->mb.i_mb_y-- )
{
row_satd[ h->mb.i_mb_y ] = 0;
for( h->mb.
i_mb_x = h->mb.i_mb_width - 1;
h->mb.i_mb_x >= 0; h->mb.i_mb_x-- )
{
...
frame->f_qp_offset[mb_index] = frame->f_qp_offset_aq[mb_index] -
strength *log2_ratio;
}
}
}

static void x264_macroblock_tree_propagate(x264_t * h, x264_frame_t * *frames,
float average_duration,
int p0,
int p1,
int b,
...
float fps_factor =
        CLIP_DURATION(frames[b]->f_duration) / (CLIP_DURATION(average_duration) * 256.0f) *
        MBTREE_PRECISION;

/* For non-reffed frames the source costs are always zero, so just memset one row and re-use it. */
if( !referenced )
memset( frames[b]
->i_propagate_cost, 0, h->mb.i_mb_width * sizeof(uint16_t));
...
mbtree_propagate_cost( buf, propagate_cost,
        frames[b]
->i_intra_cost+mb_index, lowres_costs+mb_index,
...
h->mc.
mbtree_propagate_list( h, ref_costs[0],
&mvs[0][mb_index], buf, &lowres_costs[mb_index],
bipred_weights[0], h->mb.i_mb_y, h->mb.i_mb_width, 0 );
if( b != p1 )
{
h->mc.
mbtree_propagate_list( h, ref_costs[1],
&mvs[1][mb_index], buf, &lowres_costs[mb_index],
bipred_weights[1], h->mb.i_mb_y, h->mb.i_mb_width, 1 );
}
}

if( h->param.rc.i_vbv_buffer_size && h->param.rc.i_lookahead && referenced )
x264_macroblock_tree_finish( h, frames[b], average_duration, b
== p1 ? b -
p0 :
0 );
}

static void x264_macroblock_tree(x264_t * h, x264_mb_analysis_t * a, x264_frame_t * *frames,
int num_frames,
int b_intra
)
{
...
if( b_intra )
x264_slicetype_frame_cost( h, a, frames,
0, 0, 0 );

while( i > 0 &&
IS_X264_TYPE_B( frames[i]
->i_type ))
i--;
last_nonb = i;
...
{
memset( frames[0]
->i_propagate_cost, 0, h->mb.i_mb_count * sizeof(uint16_t));
memcpy( frames[0]
->f_qp_offset, frames[0]->f_qp_offset_aq, h->mb.i_mb_count * sizeof(float));
return;
}
XCHG( uint16_t
*, frames[last_nonb]->i_propagate_cost, frames[0]->i_propagate_cost );
memset( frames[0]
->i_propagate_cost, 0, h->mb.i_mb_count * sizeof(uint16_t));
}
else
...
cur_nonb = i;
while(
IS_X264_TYPE_B( frames[cur_nonb]
->i_type ) && cur_nonb > 0 )
cur_nonb--;
if( cur_nonb < idx )
break;
...
x264_macroblock_tree_propagate( h, frames, average_duration, p0, p1, i,
0 );
}
i--;
}
x264_macroblock_tree_propagate( h, frames, average_duration, cur_nonb, last_nonb, middle,
1 );
}
else
{
...
x264_macroblock_tree_propagate( h, frames, average_duration, cur_nonb, last_nonb, i,
0 );
i--;
}
}
x264_macroblock_tree_propagate( h, frames, average_duration, cur_nonb, last_nonb, last_nonb,
1 );
last_nonb = cur_nonb;
}
...
{
x264_slicetype_frame_cost( h, a, frames,
0, last_nonb, last_nonb );
x264_macroblock_tree_propagate( h, frames, average_duration,
0, last_nonb, last_nonb, 1 );
XCHG( uint16_t
*, frames[last_nonb]->i_propagate_cost, frames[0]->i_propagate_cost );
}
...
x264_macroblock_tree_finish( h, frames[last_nonb + (bframes + 1) / 2], average_duration,
0 );
}

static int x264_vbv_frame_cost(x264_t * h, x264_mb_analysis_t * a, x264_frame_t * *frames,
int p0,
int p1,
int b
)
...
return
x264_slicetype_frame_cost_recalculate( h, frames, p0, p1, b
);
else
...
static void
x264_calculate_durations(x264_t * h, x264_frame_t * cur_frame, x264_frame_t * prev_frame,
                         int64_t * i_cpb_delay, int64_t * i_coded_fields) {
...
static void x264_vbv_lookahead(x264_t * h, x264_mb_analysis_t * a, x264_frame_t * *frames,
int num_frames,
int keyframe
)
{
...
{
h->
i_coded_fields_lookahead = frames[cur_nonb]->i_coded_fields_lookahead;
h->
i_cpb_delay_lookahead = frames[cur_nonb]->i_cpb_delay_lookahead;
}
...
frames[next_nonb]->i_planned_satd[idx] =
x264_vbv_frame_cost( h, a, frames, p0, cur_nonb, cur_nonb
);
frames[next_nonb]->i_planned_type[idx] = frames[cur_nonb]->
i_type;
frames[cur_nonb]->
i_coded_fields_lookahead = h->i_coded_fields_lookahead;
frames[cur_nonb]->
i_cpb_delay_lookahead = h->i_cpb_delay_lookahead;
x264_calculate_durations( h, frames[cur_nonb], prev_frame,
&h->i_cpb_delay_lookahead, &h->i_coded_fields_lookahead );
if( prev_frame )
{
frames[next_nonb]->f_planned_cpb_duration[prev_frame_idx] = (double)prev_frame->
i_cpb_duration *
        h
->sps->vui.i_num_units_in_tick / h->sps->vui.
i_time_scale;
}
frames[next_nonb]->f_planned_cpb_duration[idx] = (double)frames[cur_nonb]->
i_cpb_duration *
        h
->sps->vui.i_num_units_in_tick / h->sps->vui.
i_time_scale;
...
{
frames[next_nonb]->i_planned_satd[idx] =
x264_vbv_frame_cost( h, a, frames, last_nonb, cur_nonb, i
);
frames[next_nonb]->i_planned_type[idx] =
X264_TYPE_B;
frames[i]->
i_coded_fields_lookahead = h->i_coded_fields_lookahead;
frames[i]->
i_cpb_delay_lookahead = h->i_cpb_delay_lookahead;
x264_calculate_durations( h, frames[i], prev_frame,
&h->i_cpb_delay_lookahead, &h->i_coded_fields_lookahead );
if( prev_frame )
{
frames[next_nonb]->f_planned_cpb_duration[prev_frame_idx] = (double)prev_frame->
...
frames[next_nonb]->f_planned_cpb_duration[idx] = (double)frames[i]->
i_cpb_duration *
        h
->sps->vui.i_num_units_in_tick / h->sps->vui.
i_time_scale;
...
static int x264_slicetype_path_cost(x264_t * h, x264_mb_analysis_t * a, x264_frame_t * *frames,
char *path,
int threshold
)
{
...
if( path[next_nonb] == 'P' )
cost +=
x264_slicetype_frame_cost( h, a, frames, cur_nonb, next_nonb, next_nonb
);
else /* I-frame */
cost +=
x264_slicetype_frame_cost( h, a, frames, next_nonb, next_nonb, next_nonb
);
...
cost +=
x264_slicetype_frame_cost( h, a, frames, cur_nonb, next_nonb, middle
);
for(
int next_b = loc;
next_b < middle &&
...
next_b++ )
cost +=
x264_slicetype_frame_cost( h, a, frames, cur_nonb, middle, next_b
);
for(
int next_b = middle + 1;
next_b < next_nonb &&
...
next_b++ )
cost +=
x264_slicetype_frame_cost( h, a, frames, middle, next_nonb, next_b
);
}
else
for(
...
next_b++ )
cost +=
x264_slicetype_frame_cost( h, a, frames, cur_nonb, next_nonb, next_b
);
...
static void x264_slicetype_path(x264_t * h, x264_mb_analysis_t * a, x264_frame_t * *frames,
int length,
char (*best_paths)[X264_LOOKAHEAD_MAX + 1]
)
{
...
static int scenecut_internal(x264_t * h, x264_mb_analysis_t * a, x264_frame_t * *frames,
int p0,
int p1,
int real_scenecut
)
...
static int scenecut(x264_t * h, x264_mb_analysis_t * a, x264_frame_t * *frames,
int p0,
int p1,
int real_scenecut,
int num_frames,
...
curp1++ )
if( !
scenecut_internal( h, a, frames, p0, curp1,
0 ))
...
maxp1;
curp0++ )
if( origmaxp1 > i_max_search || (curp0 < maxp1 &&
scenecut_internal( h, a, frames, curp0, maxp1,
0 )))
...
assert( h
->frames.b_have_lowres );

if( !h->lookahead->last_nonb )
return;
frames[0] = h->lookahead->
last_nonb;
for(
framecnt = 0;
framecnt<i_max_search;
framecnt++ )
...
{
if( h->param.rc.b_mb_tree )
x264_macroblock_tree( h,
&a, frames, 0, keyframe );
return;
}

keyint_limit = h->param.i_keyint_max - frames[0]->i_frame + h->lookahead->i_last_keyframe - 1;
orig_num_frames = num_frames = h->param.b_intra_refresh ? framecnt : X264_MIN(framecnt,
                                                                              keyint_limit);
...
scenecut( h,
&a, frames, 0, 1, 1, orig_num_frames, i_max_search ))
{
...
frames[j]->
i_type = h->param.b_open_gop ? X264_TYPE_I : X264_TYPE_IDR;
}
...
num_frames;
j++ )
x264_slicetype_path( h,
&a, frames, j, best_paths );
...
int num_bframes = 0;
while( num_bframes < num_frames &&
IS_X264_TYPE_B( frames[num_bframes + 1]
->i_type ))
num_bframes++;
...
j < num_bframes+1; j++ )
{
if( frames[j]->i_forced_type == X264_TYPE_AUTO && IS_X264_TYPE_AUTO_OR_I(
        frames[j + 1]->i_forced_type) &&
h->param.i_scenecut_threshold &&
scenecut( h,
&a, frames, j, j+1, 0, orig_num_frames, i_max_search ))
{
frames[j]->
i_type = X264_TYPE_P;
...
x264_macroblock_tree( h,
&a, frames,
X264_MIN(num_frames, h
->param.i_keyint_max), keyframe );
...
int bframes = 0;
while( bframes < j-1 &&
IS_X264_TYPE_B( frames[j - 1 - bframes]
->i_type ))
bframes++;
last_keyframe -=
bframes;
...
if( vbv_lookahead )
x264_vbv_lookahead( h,
&a, frames, num_frames, keyframe );
...
void x264_slicetype_decide(x264_t * h) {
    ...
    for (int i = 0; i < h->lookahead->next.i_size; i++) {
        if (h->param.b_vfr_input) {
            if (lookahead_size-- > 1)
                h->lookahead->next.list[i]->i_duration = 2 *
                                                         (h->lookahead->next.list[i + 1]->i_pts -
        ...
        if (h->lookahead->next.list[i]->i_frame > h->i_disp_fields_last_frame &&
            lookahead_size > 0) {
    ...
    for (int i = 0; i <= bframes; i++) {
        ...
        if (i) {
            x264_calculate_durations(h, h->lookahead->next.list[i], h->lookahead->next.list[i - 1],
                                     &h->i_cpb_delay, &h->i_coded_fields);
            h->lookahead->next.list[0]->f_planned_cpb_duration[i - 1] =
                    (double) h->lookahead->next.list[i]->i_cpb_duration *
                    h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
        } else
            x264_calculate_durations(h, h->lookahead->next.list[i], NULL, &h->i_cpb_delay,
                                     &h->i_coded_fields);
    }
}
...
int x264_rc_analyse_slice(x264_t * h) {
    ...
    if (!IS_X264_TYPE_I(h->fenc->i_type))
        memcpy(h->fdec->i_row_satds[0][0], h->fenc->i_row_satds[0][0],
               h->mb.i_mb_height * sizeof(int));

    if (h->param.b_intra_refresh && h->param.rc.i_vbv_buffer_size &&
        ...
        for (int y = 0; y < h->mb.i_mb_height; y++) {
            ...
            for (int x = h->fdec->i_pir_start_col; x <= h->fdec->i_pir_end_col; x++, mb_xy++) {
                ...
                if (h->param.rc.i_aq_mode)
                    h->fdec->i_row_satd[y] +=
                            (diff * frames[b]->i_inv_qscale_factor[mb_xy] + 128) >> 8;
                else
