...
class VideoListener16 extends VideoListener16Base implements PreviewCallback {
    ...
    private boolean openCamera(String cameraIdStr, boolean planar) {
        ...
        LLog.d("VideoListener16", "open camera#" + cameraId);
        ...
        if (-1 == pixel_format) {
            LLog.e("VideoListener16", "failed to set preview format, camera=" + cameraId);
            return false;
        } else {
            parameters.setPreviewFormat(pixel_format);
            LLog.d("VideoListener16", "camera#" + cameraId + " preview_format=" + parameters.getPreviewFormat());
            ...
            LLog.d("VideoListener16", "camera#" + cameraId + " focus_mode=" + parameters.getFocusMode());
    ...
    public boolean start(Context context, String cameraIdStr, SurfaceHolder previewSurfaceHolder, SurfaceTexture texture, VideoEncoder encoder, int orientation) {
        try {
            if (encoder == null) {
                LLog.e("VideoListener16", "encoder is null");
                ...
                if (selected_format == -1) {
                    LLog.e("VideoListener16", "can\'t find supported color format");
            ...
            LLog.e("VideoListener16", Log.getStackTraceString(var12));
            this.setVideoCaptureState(Streamer.CAPTURE_STATE.FAILED);
            this.stop();
            return false;
        }
    ...
    protected boolean openEncoder(int colorFormat) {
        try {
            ...
            LLog.e("VideoListener16", Log.getStackTraceString(var3));
            return false;
        }
    }
    ...
    public void stop() {
        ...
        try {
            ...
            LLog.e("VideoListener16", Log.getStackTraceString(var2));
        }
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        try {
            if (null == data) {
                LLog.e("VideoListener16", "data is null");
                return;
            }

            if (null == this.encoder_.getEncoder()) {
                LLog.e("VideoListener16", "mMediaCodec is null");
                return;
            }
            ...
            LLog.e("VideoListener16", "failed to add video data into encoder buffer");
            LLog.e("VideoListener16", Log.getStackTraceString(var7));
        }
