...
public class VideoEncoder {
    ...
    private static VideoEncoder createVideoEncoder16(Streamer.Size res) {
        ...
        try {
                ...
                if (codecInfo.isEncoder()) {
                    ...
                    for (int i$ = 0; i$ < len$; ++i$) {
                        ...
                        if (supportedMime.equals("video/avc")) {
                            ...
                            if (null == cap) {
                                LLog.e("VideoEncoder", "VideoEncoder16: failed to get codec capabilities");
                                return null;
                            }

                            MediaCodec encoder = MediaCodec.createEncoderByType("video/avc");
                            if (null == encoder) {
                                LLog.e("VideoEncoder", "VideoEncoder16: failed to create video/avc encoder");
                                return null;
                            }

                            MediaFormat format = MediaFormat.createVideoFormat("video/avc", res.width, res.height);
                            if (null == format) {
                                LLog.e("VideoEncoder", "VideoEncoder16: failed to create video format");
                                encoder.release();
                                return null;
                            }
            ...
            LLog.e("VideoEncoder", Log.getStackTraceString(var12));
            return null;
        }
    }
    ...
    public static int calcBitRate(Streamer.Size resolution, int frameRate, int profile) {
        ...
        LLog.d(TAG, "compression" + compression);
