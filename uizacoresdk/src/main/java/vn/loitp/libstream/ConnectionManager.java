...
class ConnectionManager {
    ...
    ConnectionManager(StreamBuffer streamBuffer) {
        ...
        try {
            ...
            LLog.e("ConnectionManager", Log.getStackTraceString(var3));
        }
    ...
    void startConnectionThread() throws InterruptedException {
        if (null == this.connectionThread) {
            this.connectionThread = new Thread() {
                public void run() {
                    LLog.d("ConnectionManager", "connectionThread started");
                    ...
                    try {
                        while (!this.isInterrupted()) {
                            ...
                            while (i$.hasNext()) {
                                ...
                                if (selectionKey.isValid()) {
                                    ...
                                    if (null == connection) {
                                        LLog.e("ConnectionManager", "connection is null");
                                        break;
                                    }
                        ...
                        LLog.e("ConnectionManager", "connectionThread failed");
                        LLog.e("ConnectionManager", Log.getStackTraceString(var6));
                    }

                    LLog.d("ConnectionManager", "connectionThread stopped");
                }
            };
        ...
        if (timeNow - this.lastWaitingSendersCheckTime >= 200L) {
            ...
            while (i$.hasNext()) {
                ...
                if (key.isValid()) {
                    ...
                    if (null == connection) {
                        LLog.e("ConnectionManager", "null connection");
    ...
    void stopConnectionThread() {
        if (null != this.connectionThread) {
            try {
                ...
                LLog.e("ConnectionManager", "failed to stop connectionThread loop");
                LLog.e("ConnectionManager", Log.getStackTraceString(var2));
            }
    ...
    int createRtspConnection(String originalUri, Streamer.MODE mode, Streamer.Listener listener) {
        try {
            ...
            if (null == scheme) {
                LLog.e("ConnectionManager", "failed to parse scheme, uri=" + uri);
                return -1;
            }
            ...
            if (null == var17) {
                LLog.e("ConnectionManager", "failed to parse host, uri=" + uri);
                return -1;
            }
            ...
            if (scheme.equalsIgnoreCase("rtsp")) {
                ...
                if (!scheme.equalsIgnoreCase("rtmp")) {
                    LLog.e("ConnectionManager", "unsupported scheme=" + scheme);
                    return -1;
                }
                ...
                if (splittedPath.length < 5) {
                    LLog.e("ConnectionManager", "failed to get rtmp app & stream, uri=" + uri);
                    return -1;
                }
            ...
            if (null == connection) {
                LLog.e("ConnectionManager", "failed to create connection, uri=" + uri);
                return -1;
            }
            ...
            LLog.e("ConnectionManager", Log.getStackTraceString(var16));
            return -1;
        }
