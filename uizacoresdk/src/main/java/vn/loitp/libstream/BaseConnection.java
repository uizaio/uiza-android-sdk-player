...
abstract class BaseConnection {
    ...
    void Connect() throws IOException {
        ...
        try {
            ...
            LLog.e("BaseConnection", Log.getStackTraceString(var2));
            this.Close();
        }
    ...
    public void VerifyInactivity() {
        ...
        if (this.inactivity_count_ > 5) {
            LLog.d("BaseConnection", "inactivity timeout expired");
            this.Close();
        }
    ...
    public void ProcessEvent(SelectionKey selectionKey) {
        if (null != selectionKey) {
            try {
                ...
                LLog.e("BaseConnection", Log.getStackTraceString(var4));
                this.Close();
            }
    ...
    void updateAudioPacketsLost(BufferItem item) {
        ...
        if (this.audioPacketsSent_ != 0L && packetsLost != 0L) {
            LLog.d("BaseConnection", "audio frames lost " + packetsLost);
    ...
    void updateVideoPacketsLost(BufferItem item) {
        ...
        if (this.videoPacketsSent_ != 0L && packetsLost != 0L) {
            LLog.w("BaseConnection", "video frames lost " + packetsLost);
    ...
    void notifyOnStateChange(final Streamer.CONNECTION_STATE state, final Streamer.STATUS status) {
        LLog.d("BaseConnection", "notifyOnStateChange: conectionId=" + this.connectionId_ + ";state=" + state + ";status=" + status);
