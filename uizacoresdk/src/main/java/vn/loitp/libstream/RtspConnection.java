...
class RtspConnection extends BaseConnection {
    ...
    int onRecv(ByteBuffer byteBuffer) {
        ...
        if (bytesParsed < 0) {
            LLog.e("RtspConnection", "failed to parse rtsp");
            this.Close();
        ...
        while (this.getSendBufferRemaining() <= 0) {
            ...
            switch (RtspConnection.NamelessClass792357330.$SwitchMap$com$wmspanel$libstream$BufferItem$FRAME_TYPE[this.currentItem.getMessageType().ordinal()]) {
                case 1:
                    ...
                    if ((outData[0] >> 7 & 1) != 0) {
                        LLog.e("RtspConnection", "skip frame if fzb is set");
                    } else {
                        if (!this.appendVideoReport(this.currentItem)) {
                            LLog.e("RtspConnection", "failed to send video report");
                            return;
                        }

                        if (outData.length + 16 < 20000) {
                            if (!this.sendVideoFrame(this.currentItem)) {
                                LLog.e("RtspConnection", "failed to send video frame");
                                return;
                            }
                            ...
                            do {
                                ...
                                int part_len = this.sendVideoPart(this.currentItem, this.avc_part_offset);
                                if (part_len <= 0) {
                                    LLog.e("RtspConnection", "failed to send video part");
                                    return;
                                }
                ...
                case 2:
                    ...
                    if (!this.appendAudioReport(this.currentItem)) {
                        LLog.e("RtspConnection", "failed to send audio report");
                        return;
                    }

                    if (!this.sendAudioFrame(this.currentItem)) {
                        LLog.e("RtspConnection", "failed to send audio frame");
                        return;
                    }
                    ...
                    break;
                default:
                    LLog.e("RtspConnection", "unsupported frame type " + this.currentItem.getMessageType());
            }
        }
    ...
    private boolean appendVideoReport(BufferItem item) {
        if (this.avc_ntp_ts_ == -1L && this.aac_ntp_ts_ == -1L) {
            ...
            if (!this.appendReport(1, this.avc_ssrc_, this.avc_ntp_ts_, this.avc_rtp_ts_)) {
                LLog.e("RtspConnection", "failed to send report");
                return false;
            }
    ...
    private boolean sendVideoFrame(BufferItem item) {
        ...
        try {
            ...
            LLog.e("RtspConnection", Log.getStackTraceString(var6));
            return false;
        }
    }
    ...
    private int sendVideoPart(BufferItem item, int offset) {
        ...
        try {
            ...
            LLog.e("RtspConnection", Log.getStackTraceString(var12));
            return -1;
        }
    }
    ...
    private boolean appendReport(int channel, int ssrc, long ntp_ts, long rtp_ts) {
        ...
        try {
            ...
            LLog.e("RtspConnection", Log.getStackTraceString(var9));
            return false;
        }
    }
    ...
    boolean sendAudioFrame(BufferItem item) {
        ...
        if (data_len <= 2) {
            ...
            try {
                ...
                LLog.e("RtspConnection", Log.getStackTraceString(var7));
                return false;
            }
        }
    }
    ...
    private String getAuthorization(String method) {
        ...
        if (this.digestRealm_ != null && this.digestNonce_ != null) {
            try {
                ...
                LLog.e("RtspConnection", Log.getStackTraceString(var5));
            }
            ...
            try {
                ...
                LLog.e("RtspConnection", Log.getStackTraceString(var4));
            }
        }
    ...
    private void sendOptions() {
        ...
        try {
            ...
            LLog.e("RtspConnection", Log.getStackTraceString(var3));
            this.Close();
        }
        ...
        if (this.mode_ == Streamer.MODE.AUDIO_ONLY || this.mode_ == Streamer.MODE.AUDIO_VIDEO) {
            ...
            if (this.audioFormatParams_.sampleRate == -1) {
                ...
                if (this.audioFormatParams_.config_len < 1) {
                    LLog.e("RtspConnection", "failed to parse audio format params");
                    this.Close();
                    return;
                }
                ...
                if (this.audioFormatParams_.sampleRate == -1) {
                    LLog.e("RtspConnection", "failed to get sample rate, config=" + annonceRequest);
                    this.Close();
                    return;
                }
            }
            ...
            if (this.audioFormatParams_.channelCount == -1) {
                ...
                if (this.audioFormatParams_.channelCount == -1) {
                    LLog.e("RtspConnection", "failed to get channel count, config=" + annonceRequest);
                    this.Close();
                    return;
                }
            }
