...
public class DownloadTracker implements DownloadManager.Listener {
    ...
    public boolean isOffline(Uri uri) {
        return !getOfflineStreamKeys(uri).isEmpty();
    }

    public void toggleDownload(Activity activity, Class<? extends DownloadService> clazz, String name, Uri uri, String extension) {
        if (isDownloaded(uri)) {
            DownloadAction removeAction =
                    getDownloadHelper(uri, extension).getRemoveAction(Util.getUtf8Bytes(name));
            startServiceWithAction(removeAction, clazz);
        } else {
            StartDownloadDialogHelper helper =
                    new StartDownloadDialogHelper(activity, clazz, getDownloadHelper(uri, extension), name);
            helper.prepare();
        }
    }
    ...
    private void startDownload(DownloadAction action, Class<? extends DownloadService> clazz) {
        ...
        trackedDownloadStates.put(action.uri, action);
        handleTrackedDownloadStatesChanged();
        startServiceWithAction(action, clazz);
    }

    private void startServiceWithAction(DownloadAction action, Class<? extends DownloadService> clazz) {
        DownloadService.startWithAction(context, clazz, action, false);
    }

    private DownloadHelper getDownloadHelper(Uri uri, String extension) {
        int type = Util.inferContentType(uri, extension);
        LLog.e(TAG, "type = " + type);
        switch (type) {
            case C.TYPE_DASH:
    ...
    private final class StartDownloadDialogHelper
        ...
        public StartDownloadDialogHelper(
                Activity activity, Class<? extends DownloadService> clazz, DownloadHelper downloadHelper, String name) {
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (!selectedTrackKeys.isEmpty() || trackKeys.isEmpty()) {
                ...
                startDownload(downloadAction, clazz);
            }
        }
    }
}