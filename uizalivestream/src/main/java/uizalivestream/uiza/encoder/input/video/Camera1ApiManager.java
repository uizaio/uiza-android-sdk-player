package uizalivestream.uiza.encoder.input.video;
...
public class Camera1ApiManager implements Camera.PreviewCallback, Camera.FaceDetectionListener {
    ...
    public Camera.Size getCorrectCameraSize(int width, int height) {
        ...
        for (Camera.Size size : previews) {
            if (size.width == width || size.height == height) {
                LLog.d(TAG, "getCorrectCameraSize for: " + size.width + "x" + size.height);
    ...
    private boolean checkCanOpen() {
        ...
        /*if (Constants.IS_DEBUG) {
            for (Camera.Size size : previews) {
                LLog.d(TAG, "loitp camera size: " + size.width + "x" + size.height);
            }
        }*/
