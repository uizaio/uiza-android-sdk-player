    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        mAdapter = new LiveAdapter(getActivity(), dataList, new LiveAdapter.Callback() {
            ...
            public void onClick(VideoData data, int position) {
                if (data != null && data.getLastProcess() != null && data.getLastProcess().trim()
                        .equals(Constants.LAST_PROCESS_START)) {
                    ((HomeV4CanSlideActivity) getActivity()).playLiveEntityId(data.getId());
                } else {
                    UzDialogUtil
                            .showDialog1(getActivity(), "This content is not streaming now",
                                    new UzDialogUtil.Callback1() {
                                        @Override
